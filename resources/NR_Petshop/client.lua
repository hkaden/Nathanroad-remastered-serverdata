local pcode, hasPet, petcode, b, price = 0, false, nil, nil, nil
...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('pet:control', function(result, petcode)
        if result then
            hasPet = true
            pcode = petcode
        end
...
RegisterNUICallback("sit", function()
    -- print(spawned_ped, GetEntityHealth(spawned_ped), 'spawned_ped')
    sit(spawned_ped)
    ESX.UI.Notify("success", "成功")
end)
...
function spawnped(model)
    if spawned_ped == nil then
        ...
        AddTextComponentString(tostring("我的寵物"))
        EndTextCommandSetBlipName(blip)
        NetworkRegisterEntityAsNetworked(spawned_ped)
        GiveWeaponToPed(spawned_ped, `WEAPON_ANIMAL`, 200, true, true);
        pedfallow()
...
RegisterCommand(Config.openmenu, function()
    if hasPet then
        ...
        ESX.UI.Notify("success", "你沒有寵物")
    end
end)

RegisterNetEvent("cl:getname")
AddEventHandler("cl:getname", function(name, pricee)
    SendNUIMessage({
        action = "update",
        price = pricee,
        name = name
