...
Citizen.CreateThread(function()
    ...
    -- if Config.Framework == "esx" then
        frameworkObject.RegisterServerCallback("codem-blvckhudv2:GetMoney", function(source, cb, moneytype, jobName)
            local src = source
            ...
            elseif moneytype == "society" then
                if jobName then
                    cb(exports.NR_Banking:GetSharedAccounts("society_".. jobName))
                else
                    cb(0)
                end
            else
        ...
        frameworkObject.RegisterServerCallback("codem-blvckhudv2:GetAllMoney", function(source, cb, jobName)
            local src = source
            local Player = frameworkObject.GetPlayerFromId(src)
            local money, bank, society = Player.getMoney(), Player.getAccount('bank').money, exports.NR_Banking:GetSharedAccounts("society_".. jobName)
            cb(money, bank, society)
        end)
    -- end
end)
preferences = {}
