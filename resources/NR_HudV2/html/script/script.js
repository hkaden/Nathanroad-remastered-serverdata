...
const app = new Vue({
    ...
    methods: {
        ...
        UpdateJob(data) {
            this.joblabel = data.joblabel
            this.grade_label = data.grade_label
            this.can_access_society = data.can_access_society
            
            $(".job_label").css("color", data.jobColor)
            $(".grade_label").css("color", data.jobColor)
            $(".society_money").css("color", data.jobColor)
            $(".job_label").css("text-shadow", "0px 0.25rem 0.813rem " + data.jobColor)
            $(".grade_label").css("text-shadow", "0px 0.25rem 0.813rem " + data.jobColor)
            $(".society_money").css("text-shadow", "0px 0.25rem 0.813rem " + data.jobColor)
            // $(".job_label").css(["color", "text-shadow"], [data.jobColor, "0px 4px 13px " + data.jobColor])
            // $(".grade_label").css(["color", "text-shadow"], [data.jobColor, "0px 4px 13px " + data.jobColor])
            // $(".society_money").css(["color", "text-shadow"], [data.jobColor, "0px 4px 13px " + data.jobColor])
        },
        SetSociety(val) {
            this.society_money = val
        },
        SetLocales(val) {
            this.locales = val
        },
...
window.addEventListener("message", function (event) {
    ...
    switch (item.type) {
        ...
        case "update_money":
            ...
            break
        case "update_society_money":
            app.SetSociety(item.money)
            break
        case "watermark_text":
        ...
        case "set_force_hide":
            ...
            break
        case "update_job":
            console.log(item.data, "update_job nui call back");
            app.UpdateJob(item.data)
            break
