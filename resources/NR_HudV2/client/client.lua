...
if Config.Voice == 'mumble' then
   ...
   AddEventHandler('SaltyChat_MicEnabledChanged', function(isMicrophoneMuted)
        if isMicrophoneMuted then
            SendNUIMessage({
                type = 'toggle_muted',
                toggle = true
...
if Config.EnableCruise then
    ...
    RegisterKeyMapping('cruise', '巡航定速', 'keyboard', Config.DefaultCruiseControlKey)
    local cruiseSpam = 0
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1700)
...
if Config.EnableSeatbelt then
    ...
    RegisterKeyMapping('seatbelt', '繫上/鬆開安全帶', 'keyboard', Config.DefaultSeatbeltControlKey)
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1500)
...
if Config.EnableEngineToggle then
    ...
    -- RegisterKeyMapping('engine', '開關引擎', 'keyboard',  Config.VehicleEngineToggleKey)
end
