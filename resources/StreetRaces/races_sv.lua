    ...
    for index, race in pairs(races) do
        ...
        if source == race.owner and time < race.startTime then
            ...
            for _, player in pairs(race.players) do
                ...
                Inventory:AddItem(player, 'money', race.amount)
...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
    ...
    local playerMoney = Inventory:Search(src, 'count', 'money')
    ...
    if playerMoney >= amount then
        ...
        -- TriggerEvent('esx:sendToDiscord', 16753920, "街頭賽車", xPlayer.name .. " 以 $".. amount .. "參加了 "  .. "賽池總共有: ".. race.prize .. os.date(), "", "https://discordapp.com/api/webhooks/669820726773284865/Ov6TRsTcBjDTB55Glp0-braSv1sznUJOcSByHt_ow9ADpsYj_yFIRqJjw4n5EzAUIclq")

        -- Add player to race and send join event back to client
        table.insert(races[index].players, src)
        TriggerClientEvent("StreetRaces:joinedRace_cl", src, index)
    else
        -- Insufficient money, send notification back to client
        local msg = "你的金錢不足，所以無法參加比賽"
    ...
    for index, player in pairs(players) do
        if src == player then 
            ...
            if race.finishTime == 0 then
                ...
                -- addMoney(src, race.prize)
                Inventory:AddItem(src, 'money', race.prize)
                -- TriggerEvent('esx:sendToDiscord', 16753920, "街頭賽車", xPlayer.name .. " 贏得了 $".. race.prize .. "賽池獎金: " .. os.date(), "", "https://discordapp.com/api/webhooks/669820726773284865/Ov6TRsTcBjDTB55Glp0-braSv1sznUJOcSByHt_ow9ADpsYj_yFIRqJjw4n5EzAUIclq")

                -- Send winner notification to players
                for _, pSource in pairs(players) do
                    if pSource == src then
                        local msg = ("你贏得了 [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                        -- TriggerClientEvent('chatMessage', source, "[街頭賽事]", {238, 198, 78}, msg)
                    elseif config_sv.notifyOfWinner then
                        local msg = ("%s 贏得了 [%02d:%06.3f]"):format(getName(src), timeMinutes, timeSeconds)
                        -- notifyPlayer(pSource, msg)
                        TriggerClientEvent('chatMessage', -1, "[街頭賽事]", {238, 198, 78}, msg)
                    end
                end
            else
                -- Loser, send notification to only the player
                local msg = ("%s 贏得了 [%02d:%06.3f]"):format(getName(src), timeMinutes, timeSeconds)
                -- local msg = ("你輸了 [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                TriggerClientEvent('chatMessage', -1, "[街頭賽事]", {238, 198, 78}, msg)
                -- notifyPlayer(src, msg)
            end
