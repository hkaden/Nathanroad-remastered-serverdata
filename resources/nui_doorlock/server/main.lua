if not ESX then
	...
	AddEventHandler('nui_doorlock:newDoorCreate', function(config, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
		...
		local newDoor = {}
		if jobs[1] then auth = tostring("'"..jobs[1].."'") end
		if jobs[2] then auth = auth..', '..tostring("'"..jobs[2].."'") end
		if jobs[3] then auth = auth..', '..tostring("'"..jobs[3].."'") end
		if jobs[4] then auth = auth..', '..tostring("'"..jobs[4].."'") end

		if auth then newDoor.authorizedJobs = { auth } end
		if item then newDoor.items = { item } end
		newDoor.locked = doorLocked
		newDoor.distance = maxDistance
		-- newDoor.slides = slides
		if not doubleDoor then
			-- newDoor.garage = garage
			newDoor.objName = model
			-- newDoor.objHeading = heading
			newDoor.objCoords = coords
			newDoor.textCoords = coords
			-- newDoor.fixText = false
		else
			newDoor.doors = {
				{objName = model[1], objCoords = coords[1]},
				{objName = model[2], objCoords = coords[2]}
			}
			newDoor.textCoords = coords[1]
		end
			-- newDoor.audioRemote = false
			newDoor.pickable = false
		...
		for k,v in pairs(newDoor) do
			...
			elseif k == 'doors' then
				local doorStr = {}
				for i=1, 2 do
					table.insert(doorStr, ('	{objName = %s, objCoords = %s}'):format(model[i], coords[i]))
				end
