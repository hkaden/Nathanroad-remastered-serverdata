...
local GlobalCurrentDurability = 100
local cacheSlot = 0
...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		...
		-- 	vehicle,
		-- 	F1UsageClasses,
		-- 	GetVehicleFuelLevel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10,
		-- 	'vehicle'
		-- )
		-- print(
		-- 	Round(GetVehicleCurrentRpm(vehicle), 1), 
		...
		-- )
		SetFuel(vehicle, GetVehicleFuelLevel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.2) / 10)
	end
end
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	ESX.TriggerServerCallback('fuel:server:GetPerolcanLevel', function(slot, result)
		currentDurability = result
		cacheSlot = slot
	end)
	...
	while isFueling do
		...
		local fuelToAdd = 1
		local extraCost = fuelToAdd / 0.05

		if not pumpObject then
			if currentDurability - fuelToAdd >= 0 then
				currentFuel = oldFuel + fuelToAdd
				-- TriggerServerEvent('fuel:addammo')
				currentDurability = math.floor(currentDurability - fuelToAdd)
				-- SetPedAmmo(ped, 883325847, math.floor(currentDurability - fuelToAdd * 100))
			else
				isFueling = false
			end
		else
	...
	if not pumpObject then
		TriggerServerEvent('fuel:server:SetPerolcanLevel', currentDurability, cacheSlot)
	else
	...
	while true do
		if isFueling then
			...
			if GetSelectedPedWeapon(ped) == 883325847 then
				ESX.TriggerServerCallback('fuel:server:GetPerolcanLevel', function(slot, result)
					GlobalCurrentDurability = result
					cacheSlot = slot
				end)
			end
		end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				local vehicleCoords = GetEntityCoords(vehicle)
				if DoesEntityExist(vehicle) and #(pos - vehicleCoords) < 3.5 then
					sleep = 4
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetSelectedPedWeapon(ped) == 883325847 then
							...
							if GlobalCurrentDurability < 1 then
								canFuel = false
							end
						end
					...
					if currentCash >= Config.JerryCanCost then
						if GetSelectedPedWeapon(ped) ~= 883325847 then
							...
							if IsControlJustReleased(0, 38) then
								...
								-- end
								TriggerServerEvent('fuel:giveweapon', Config.JerryCanCost)
								-- TriggerServerEvent('fuel:pay', Config.JerryCanCost)
								-- SetPedAmmo(ped, 883325847, 4500)
								currentCash = ESX.GetPlayerData().money
							end
						-- elseif GetSelectedPedWeapon(ped) == 883325847 then
						-- 	if Config.UseESX then
