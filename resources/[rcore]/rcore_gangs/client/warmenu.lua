...
local defaultStyle = {
	...
	titleBackgroundColor = { 255, 255, 255, 255 },
...
function WarMenu.ToolTip(text, width, flipHorizontal)
	...
	width = getStyleProperty('width')

	local x = nil
	if not flipHorizontal then
		x = getStyleProperty('x') + width / 2
	else
	...
	local y = getStyleProperty('y') + titleHeight + buttonHeight + height / 2
...
function WarMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	local selectedItem = items[currentIndex]['label']
...
function WarMenu.Display(ignoreInput)
	if currentMenu then
