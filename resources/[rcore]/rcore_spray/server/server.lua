...
AddEventHandler('rcore_spray:addSpray', function(spray)
    ...
    if spray.text or spray.image then
        ...
        local count = Inventory:Search(src, "count", "spray")

        if count > 0 then
            Inventory:RemoveItem(src, "spray", 1)
            spray.identifier = GetPlayerIdentifiers(src)[1]
            AddSpray(src, spray)
        else
            ShowNotification(src, Config.Text.NEED_SPRAY)
        end
    end
end)
...
function HasSpray(serverId, cb)
    local count = Inventory:Search(serverId, "count", "spray")
    cb(count > 0)
end

function SafeGetItem(xPlayer, name)
    local item = Inventory:Search(xPlayer.source, "slots", name)
