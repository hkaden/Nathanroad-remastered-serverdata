...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/e', '做動作', {{ name="動作名稱", help="/emotebinds 可以查看走路姿勢列表"}})
    TriggerEvent('chat:addSuggestion', '/e', '做動作', {{ name="動作名稱", help="/emotebinds 可以查看走路姿勢列表"}})
    TriggerEvent('chat:addSuggestion', '/emote', '做動作', {{ name="動作名稱", help="/emotebinds 可以查看走路姿勢列表"}})
    if Config.SqlKeybinding then
      TriggerEvent('chat:addSuggestion', '/emotebind', '綁定動作至數字鍵', {{ name="數字鍵", help="num4, num5, num6, num7. num8, num9. 數字鍵4-9!"}, { name="動作名稱", help="/emotebinds 可以查看走路姿勢列表"}})
      TriggerEvent('chat:addSuggestion', '/emotebinds', '查看你已綁定的熱鍵')
    end
    TriggerEvent('chat:addSuggestion', '/emotemenu', '開啟動作選單')
    TriggerEvent('chat:addSuggestion', '/emotes', '列出所有動作名稱.')
    TriggerEvent('chat:addSuggestion', '/walk', '更變走路姿勢.', {{ name="姿勢", help="/walks 可以查看走路姿勢列表"}})
    TriggerEvent('chat:addSuggestion', '/walks', '查看走路姿勢列表.')
end)
...
RegisterNetEvent('Emote:OnEmotePlay')
AddEventHandler('Emote:OnEmotePlay', function(name)
  if DP.Emotes[name] ~= nil then
    if OnEmotePlay(DP.Emotes[name]) then end return
  elseif DP.Dances[name] ~= nil then
    if OnEmotePlay(DP.Dances[name]) then end return
