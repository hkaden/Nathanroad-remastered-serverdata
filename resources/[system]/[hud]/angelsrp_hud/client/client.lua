...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerServerEvent('angelsrp_hud:getServerInfo')
...
-- Overall Info
RegisterNetEvent('angelsrp_hud:setInfo')
AddEventHandler('angelsrp_hud:setInfo', function(info)
	...
	--[[if Config.ui.showSranje == true then
        ...
        TriggerEvent('esx_status:getStatus', 'shit', function(status)
            if status.val < 600000 then
                ...
                ESX.UI.Notify('info', '~r~Ti nemas potrebu za velikom nuzdom')
            end
        end)
	...
	if Config.ui.showPisanje == true then
        ...
        TriggerEvent('esx_status:getStatus', 'pee', function(status)
            if status.val < 600000 then
                ...
                ESX.UI.Notify('info', '~r~Ti nemas potrebu za malom nuzdom')
            end
        end)
	end--]]
...
-- 			TriggerEvent('angelsrp_hud:setCarSignalLights', vehicleSignalIndicator)
-- 		end
...
AddEventHandler('angelsrp_hud:setCarSignalLights', function(status)
	...
	TriggerServerEvent('angelsrp_hud:syncCarLights', status)
...
RegisterNetEvent('angelsrp_hud:syncCarLights')
AddEventHandler('angelsrp_hud:syncCarLights', function(driver, status)
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	...
	if ESX.PlayerData.job and (ESX.PlayerData.job.grade_name == 'boss') and ( 'society_' .. ESX.PlayerData.job.name == society or ( ESX.PlayerData.job.name == 'secworker' and society == 'secworker' ) )  then
		SendNUIMessage({ action = 'setMoney', id = 'society', value = money })
	end
end)
