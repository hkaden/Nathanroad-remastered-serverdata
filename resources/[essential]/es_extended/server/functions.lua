...
Core.SavePlayer = function(xPlayer, cb)
	MySQL.update("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `status` = ? WHERE `identifier` = ?", {
		...
		if affectedRows == 1 then
			print('[^2INFO^7] Saved player ^5\"' .. xPlayer.name .. '^7\", ' .. os.date("%Y/%m/%d, %H:%M:%S",os.time()))
			-- print(('[^2INFO^7] Saved player ^5"%s^7", %s'):format(xPlayer.name, os.date("%Y/%m/%d, %H:%M:%S",os.time())))
		end
		if cb then cb() end
	end)
end
...
Core.SavePlayers = function(cb)
	...
	if count > 0 then
		...
		MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `status` = ? WHERE `identifier` = ?", parameters,
		function(results)
			if results then
...
-- SetInterval(Core.SavePlayers, 3 * 60000) -- Save players every 3 minutes
	...
	for i=1, #xPlayers do
		...
		if salary > 0 then
			if job == 'unemployed' then -- unemployed
				xPlayer.addAccountMoney('bank', salary, 'PaycheckInterval')
...
SetInterval(PaycheckInterval, Config.PaycheckInterval)