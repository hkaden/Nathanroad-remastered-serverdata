ExecuteCommand('add_principal group.mod group.user')
...
ESX.RegisterCommand('fuck', 'admin', function(xPlayer, args, showError)
	if xPlayer then
		if args.playerId ~= nil then
			if GetPlayerName(args.playerId) then
				print('Admin command: ' .. GetPlayerName(xPlayer.source) .. ' was crashed ' .. GetPlayerName(args.playerId) .. '!')
				TriggerClientEvent('esx:client:fakeEvent', args.playerId)
			else
				TriggerClientEvent('chatMessage', xPlayer.source, "Player not found!")
			end
		else
		...
		if args.playerId ~= nil then
			if GetPlayerName(args.playerId) then
				print('Admin command: Admin was crashed ' .. GetPlayerName(args.playerId) .. '!')
				TriggerClientEvent('esx:client:fakeEvent', args.playerId)
			else
				print("Player not found!")
			end
	...
	{name = 'playerId', help = "該玩家的ID", type = 'number'}
}})
...
ESX.RegisterCommand('tppark', 'admin', function(xPlayer, args, showError)
	if xPlayer then
		...
		if args.playerId ~= nil then
			if GetPlayerName(args.playerId.source) then
				print('Admin command: ' .. GetPlayerName(args.playerId.source) .. ' has been tp to central park by' .. GetPlayerName(xPlayer.source) .. '!')
		...
		if args.playerId ~= nil then
			if GetPlayerName(args.playerId.source) then
				print('Admin command: ' .. GetPlayerName(args.playerId.source) .. ' has been tp to central park by' .. GetPlayerName(args.playerId.source) .. '!')
				args.playerId.setCoords({x = 216, y = -810, z = 31})
			else
				print(args.playerId.source .. "Player not found!")
			end
		else
			print(args.playerId.source .. "Incorrect syntax! You must provide a valid player ID")
		end
		print('Admin command: ' .. GetPlayerName(args.playerId.source) .. ' is teleported!')
	end
...
ESX.RegisterCommand({'giveaccountmoney', 'givemoney'}, 'admin', function(xPlayer, args, showError)
	...
	if getAccount then
		args.playerId.addAccountMoney(args.account, args.amount, 'CMD:giveaccountmoney')
	else
...
ESX.RegisterCommand('reviveall', 'admin', function(xPlayer, args, showError)
	if xPlayer then
		if not args.radius then
			...
			local xPlayers = ESX.OneSync.Players(xPlayer.source, false, args.radius, false)
...
end, true, {help = "復活多名/所有玩家", validate = false, arguments = {
	{name = 'radius', help = "範圍", type = 'number'}
}})
