...
ESX.TriggerServerCallback = function(name, cb, ...)
	...
	print('Debug Print: ESX.TriggerServerCallback: ' .. name)
...
ESX.UI.Notify = function(textype, text, title, time)
	local ttype = textype ~= nil and textype or "info"
	local ttitle = title ~= nil and title or ""
	local ttime = time ~= nil and time or 5000
	exports['NR_Notify']:Alert(ttitle, text, ttype, ttime)
end
...
ESX.Game.SpawnVehicle = function(vehicle, coords, heading, cb, networked)
	...
	CreateThread(function()
		...
		vehicle = CreateVehicle(model, vector.xyz, heading, networked, false)
...
ESX.Game.SetVehicleMaxMods = function(vehicle)
	local props = {
		modEngine       = 3,
		modBrakes       = 2,
		modTransmission = 2,
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	if NetworkGetEntityIsNetworked(vehicle) and NetworkGetEntityOwner(vehicle) ~= PlayerId() then error(("Unable to set vehicle properties for '%s' (client is not entity owner)"):format(vehicle)) end
	...
	if props.modSubwoofer then ToggleVehicleMod(vehicle, 19, props.modSubwoofer) end
	if props.modHydraulics then ToggleVehicleMod(vehicle, 21, props.modHydraulics) end
