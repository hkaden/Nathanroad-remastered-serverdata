local default = { 'en', 'fr', 'de', 'it', 'es', 'pt-BR', 'pl', 'ru', 'ko', 'zh-TW', 'ja', 'es-MX', 'zh-CN', 'zh-HK' }
local userLocale = GetResourceKvpString('locale') or default[GetCurrentLanguage() + 1]

local function loadLocale(locale, cb)
	if cb then cb(1) end
	local JSON = LoadResourceFile('NR_lib', ('locales/%s.json'):format(locale)) or LoadResourceFile('NR_lib', ('locales/hk.json'):format(locale))
