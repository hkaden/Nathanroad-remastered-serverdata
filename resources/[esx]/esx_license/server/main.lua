...
function RemoveLicense(target, type, cb)
	...
	if xPlayer then
		MySQL.query('DELETE FROM user_licenses WHERE type = @type AND owner = @owner', {
			['@type'] = type,
			['@owner'] = xPlayer.identifier
		}, function(rowsChanged)
			if cb then
...
ESX.RegisterUsableItem("weapon_licenes", function(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	local invItem = xPlayer.getInventoryItem('weapon_licenes')
	if invItem.count > 0 then
		xPlayer.removeInventoryItem('weapon_licenes', 1)
		-- AddLicense(source, 'weapon', true)
