...
AddEventHandler('esx_holdupbank:robberycomplete', function(robb)
	holdingup = false
	ESX.UI.Notify('info', _U('robbery_complete') .. Banks[bank].reward)
	bank = ""
	secondsRemaining = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						if IsControlJustReleased(1, 51) then
							ESX.TriggerServerCallback('esx_holdupbank:getEvent', function(event_is_running)
		...
		if holdingup then
			...
			drawTxt(0.68, 1.35, 1.0,1.0,0.4, _U('robbery_of') .. secondsRemaining .. _U('seconds_remaining'), 255, 255, 255, 255)
