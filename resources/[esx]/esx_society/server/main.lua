...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			TriggerEvent('esx:sendToDiscord', 16753920, "職業公款", xPlayer.name .. " 已從 " .. xPlayer.job.label .. " 取出了 $" .. amount .. " 公款 " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732702387571261473/IKM_BBTxhNN65hjSb49LTvJUZvB-5aregyaTe_aR4vN6fx4Jjz8xKgtDjx-YeaRHNddK")
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		TriggerEvent('esx:sendToDiscord', 16753920, "職業公款", xPlayer.name .. " 已從 " .. xPlayer.job.label .. " 存入了 $" .. amount .. " 公款 " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732702387571261473/IKM_BBTxhNN65hjSb49LTvJUZvB-5aregyaTe_aR4vN6fx4Jjz8xKgtDjx-YeaRHNddK")
		TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('have_deposited', ESX.Math.GroupDigits(amount)))
	else
		TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('invalid_amount'))
	end
end)
...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if amount and amount > 0 and account.money >= amount then
		...
		TriggerClientEvent('esx_xp:Add', source, amount/350)
		TriggerEvent('esx:sendToDiscord', 16753920, "黑錢記錄", xPlayer.name .. " 已從 " .. xPlayer.job.label .. " 洗了 $" .. amount .. " 黑錢 " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732702512813047910/9xE5TB3G7ZFYgVJZwmjICk7h0HrJgidOlt0jMwaE-nTzDLKZxkpALuDLAemvAZeQ0alv")
		...
		MySQL.update('INSERT INTO society_moneywash (identifier, society, amount) VALUES (@identifier, @society, @amount)', {
			...
			TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('you_have', ESX.Math.GroupDigits(amount)))
		end)
	else
		TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('invalid_amount'))
	end
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('esx:Notify', xTarget.source, 'info', _U('you_have_been_hired', job))
			elseif type == 'promote' then
				TriggerClientEvent('esx:Notify', xTarget.source, 'info', _U('you_have_been_promoted'))
			elseif type == 'fire' then
				TriggerClientEvent('esx:Notify', xTarget.source, 'info', _U('you_have_been_fired', xTarget.getJob().label))
			end
	...
	MySQL.query('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerEvent('esx:sendToDiscord', 16753920, "黑錢成功記錄", "黑錢成功記錄, " .. xPlayer .. ", 已成功, 洗了 $, " .. result[i].amount .. ", 黑錢, " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732702512813047910/9xE5TB3G7ZFYgVJZwmjICk7h0HrJgidOlt0jMwaE-nTzDLKZxkpALuDLAemvAZeQ0alv")
			end)

			-- send notification if player is online
			if xPlayer then
				TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)))
			end
...
TriggerEvent('cron:runAt', 2, 58, WashMoneyCRON)
