...
local chickenSellSpawned = false
...
Citizen.CreateThread(function()
	...
	refreshBlips()
	local spawnCoords = vector3(-591.46, -892.62, 24.94)
	spawnSellPed(spawnCoords, 85.15)
end)
...
-- Spawn NPC
function spawnSellPed(coords, heading)
	if not chickenSellSpawned then
		...
		ChickenBoss = CreatePed(5, hash, coords, heading, false, false)
		FreezeEntityPosition(ChickenBoss, true)
		SetEntityInvincible(ChickenBoss, true)
		SetBlockingOfNonTemporaryEvents(ChickenBoss, true)
		exports['qtarget']:AddEntityZone('ChickenBoss', ChickenBoss, {
			name = "ChickenBoss",
			debugPoly = false,
			useZ = true
		}, {
			options = {{
				event = "chickenjob:sellChicken",
		...
		chickenSellSpawned = true
	end
end

RegisterNetEvent('chickenjob:sellChicken')
AddEventHandler('chickenjob:sellChicken',function()
    TriggerServerEvent('chickenjob:checkChickenAmt')
end)

function OpenMenu()
	onDuty = true
	ESX.UI.Notify('success', '你可以工作了')
...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			ESX.UI.Notify('info', _U('foot_work'))
		else
			Wait(5000)
	...
	elseif zone.Type == "vehspawner" then
		...
		if ESX.Game.IsSpawnPointClear(spawnPoint.Pos, 5.0) then
			...
			ESX.UI.Notify('info', _U('spawn_blocked'))
		end
							...
							if playerPed == driverPed then
								...
								for i=1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										local giveBack = ESX.Math.Round(vehicleHealth / 1000, 2)
								...
								ESX.UI.Notify('info', _U('not_your_vehicle'))
							end
...
function nextStep(gps)
	if gps ~= 0 then
		...
		ESX.UI.Notify('info', _U('next_point'))
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and isInPublicMarker then
			ESX.Game.Teleport(PlayerPed, position)
			isInPublicMarker = false
		end
