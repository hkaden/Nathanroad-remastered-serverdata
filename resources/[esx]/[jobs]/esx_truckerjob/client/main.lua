	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('job_wear'), value = 'citizen_wear'}
			}
		},
		function(data, menu)
...
function retourcamion_non()
	...
	if livraisonnombre >= Config.MaxDelivery then
		exports['mythic_notify']:DoHudText('inform', _U('need_it'))
	else
		exports['mythic_notify']:DoHudText('inform', _U('ok_work'))
		nouvelledestination()
	end
end
...
function retourcamionperdu_non()
	exports['mythic_notify']:DoHudText('inform', _U('scared_me'))
end
...
function retourcamionannulermission_non()	
	exports['mythic_notify']:DoHudText('inform', _U('resume_delivery'))
end
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			exports['mythic_notify']:DoHudText('inform', _U('not_delivery'))
			exports['mythic_notify']:DoHudText('inform', _U('pay_repair'))
			exports['mythic_notify']:DoHudText('inform', _U('repair_minus')..argentretire)
			TriggerServerEvent("esx_truckerjob:pay", amount)
			livraisonTotalPaye = 0
		else
			if argentretire <= 0 then
				exports['mythic_notify']:DoHudText('inform', _U('shipments_plus')..livraisonTotalPaye)
				TriggerServerEvent("esx_truckerjob:pay", amount)
				livraisonTotalPaye = 0
			else
				exports['mythic_notify']:DoHudText('inform', _U('shipments_plus')..livraisonTotalPaye)
		...
		if livraisonTotalPaye ~= 0 and amount <= 0 then
			exports['mythic_notify']:DoHudText('inform', _U('truck_state'))
			livraisonTotalPaye = 0
		else
			if argentretire <= 0 then
				exports['mythic_notify']:DoHudText('inform', _U('shipments_plus')..livraisonTotalPaye)
...
function donnerlapayesanscamion()
	...
	if livraisonTotalPaye == 0 then
		exports['mythic_notify']:DoHudText('inform', _U('no_delivery_no_truck'))
		exports['mythic_notify']:DoHudText('inform', _U('truck_price')..argentretire)
					TriggerServerEvent("esx_truckerjob:pay", amount)
		livraisonTotalPaye = 0
	else
		if amount >= 1 then
...
function MissionLivraisonSelect()
	...
	if MissionRegion == 1 then -- Los santos
		...
		elseif MissionNum == 4 then destination = Config.Livraison.Delivery4BC namezone = "Delivery4BC" namezonenum = 4 namezoneregion = 1
		elseif MissionNum == 5 then destination = Config.Livraison.Delivery5BC namezone = "Delivery5BC" namezonenum = 5 namezoneregion = 1
		elseif MissionNum == 6 then destination = Config.Livraison.Delivery6BC namezone = "Delivery6BC" namezonenum = 6 namezoneregion = 1
		elseif MissionNum == 7 then destination = Config.Livraison.Delivery7BC namezone = "Delivery7BC" namezonenum = 7 namezoneregion = 1
		end
...
function MissionLivraisonLetsGo()
	...
	if MissionRegion == 1 then -- Los santos
		exports['mythic_notify']:DoHudText('inform', _U('meet_ls'))
	elseif MissionRegion == 2 then -- Blaine County
		exports['mythic_notify']:DoHudText('inform', _U('meet_bc'))
	elseif MissionRegion == 0 then -- au cas ou
		exports['mythic_notify']:DoHudText('inform', _U('meet_del'))
	end
...
function MissionLivraisonStopRetourDepot()
	...
	exports['mythic_notify']:DoHudText('inform', _U('return_depot'))
