...
-- TriggerEvent('esx_phone:registerNumber', 'cardealer', _U('dealer_customers'), true, true)
-- TriggerEvent('esx_society:registerSociety', 'cardealer', _U('car_dealer'), 'society_cardealer', 'society_cardealer', 'society_cardealer', {type = 'public'})
...
--             -- TriggerClientEvent('chatMessage', source, "系統", {255, 0, 0}, "錯誤的玩家ID")
-- 			TriggerClientEvent('esx:Notify', source, 'info', "錯誤的玩家ID")
-- 			return
-- 	end
...
-- 				local VehiclesModelJson = json.decode(result[1].vehicle)
-- 				VehiclesModel = VehiclesModelJson.model
...
-- 				print(VehiclesModel.."123")
-- 				-- local targetPlayerID = ESX.GetPlayerFromIdentifier(args[1]).identifier
-- 				CarOwner = playerName
-- 				print("Car Transfer ", xPlayer.identifier, CarOwner)
-- 				-- TriggerEvent('esx:sendToDiscord', 16753920, "載具轉移 ", xPlayer.name .. ", " .. CarOwner .. ", 將, " .. plate .. ", 轉移至, " .. targetSteamID .. " " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732692718630994000/8sey1e5xCLViG-qR4QxZb6CqaR_vrZ52Lohm0Bo9JwUAmAW_RwgX_qqBSGEyyyX3w8hx")
-- 				if myID == CarOwner then
-- 					print("Transfered")

-- 					data = {}
-- 					-- TriggerEvent('esx:sendToDiscord', 16753920, "載具轉移", xPlayer.name .. ", " .. CarOwner .. ", 將, " .. plate .. ", 轉移給, " .. targetSteamName .. ", " .. targetSteamID .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discord.com/api/webhooks/754903857540235284/q1LYRh--8oBvOoTMOj4M8tZSNixu4Kpr4I-WA65q8RKeMwPfxs_XmNf6MYfgS23pZhlC")
-- 						TriggerClientEvent('chatMessage', other, "^2" .. myName .. "將^4車輛 ^*^1" .. plate .. "^r^4轉移給你^*")
-- 						MySQL.update("UPDATE owned_vehicles SET owner=@owner WHERE plate=@plate", {['@owner'] = targetSteam, ['@plate'] = plate})
-- 						TriggerClientEvent('chatMessage', source, "^4你成功將" .. plate .. "^*^3轉移給^2".. targetSteamName)
-- 				else
-- 					print("Did not transfer")
-- 					TriggerClientEvent('chatMessage', source, "^*^1你不是這台車的擁有者")
-- 				end
-- 				TriggerEvent('esx:sendToDiscord', 16753920, "載具轉移 ", xPlayer.identifier .. ", " .. xPlayer.name .. ", 將 Model:, " .. VehiclesModel .. ", " .. plate .. ", 轉移至, " .. targetSteam .. ", " .. targetSteamName .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discord.com/api/webhooks/754903857540235284/q1LYRh--8oBvOoTMOj4M8tZSNixu4Kpr4I-WA65q8RKeMwPfxs_XmNf6MYfgS23pZhlC")
-- 			else
-- 				TriggerClientEvent('chatMessage', source, "^1^*錯誤: ^r^0這台車不存在或車牌輸入錯誤")
--             end
--         end
--     )
-- end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps, model)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	MySQL.update('INSERT INTO owned_vehicles (owner, plate, vehicle, model) VALUES (@owner, @plate, @vehicle, @model)',
	{
		...
		TriggerClientEvent('esx:Notify', source, 'info', _U('vehicle_belongs', vehicleProps.plate))
	end)
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
	...
	MySQL.update('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		TriggerClientEvent('esx:Notify', playerId, 'info', _U('vehicle_belongs', vehicleProps.plate))
	end)
end)
...
AddEventHandler('esx_vehicleshop:sellVehicle', function (vehicle)
	MySQL.query('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		['@vehicle'] = vehicle
	}, function (result)
...
ESX.RegisterServerCallback('esx_vehicleshop:getSoldVehicles', function (source, cb)
	...
	MySQL.query('SELECT * FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function (inventory)
		...
		if count > 0 and item.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				TriggerClientEvent('esx:Notify', source, 'info', _U('player_cannot_hold'))
			else
				inventory.removeItem(itemName, count)
				xPlayer.addInventoryItem(itemName, count)
				TriggerClientEvent('esx:Notify', source, 'info', _U('have_withdrawn', count, item.label))
			end
		else
			TriggerClientEvent('esx:Notify', source, 'info', _U('not_enough_in_society'))
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function (inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerClientEvent('esx:Notify', source, 'info', _U('have_deposited', count, item.label))
		else
			TriggerClientEvent('esx:Notify', source, 'info', _U('invalid_amount'))
		end
	end)
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		cb(true)
		local whData = {
			message = xPlayer.identifier .. ', ' .. xPlayer.name .. ", 以 $," .. vehicleData.price .. ", 購買了, " .. vehicleModel .. ", " .. Vehicles[i].model,
			sourceIdentifier = xPlayer.identifier,
			event = 'esx_vehicleshop:buyVehicle'
		}
		local additionalFields = {
			_type = 'VehicleShop:buyVehicle',
			_playerName = xPlayer.name, -- GetPlayerName(PlayerId())
			_model = Vehicles[i].model,
			_modelName = vehicleModel,
			_price = vehicleData.price
		}
		TriggerEvent('NR_graylog:createLog', whData, additionalFields)
		-- TriggerEvent('esx:sendToDiscord', 16753920, "車輛購買 - 車輛", xPlayer.name .. ", 以 $," .. vehicleData.price .. ", 購買了, " .. vehicleModel .. ", " .. Vehicles[i].model .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/732692718630994000/8sey1e5xCLViG-qR4QxZb6CqaR_vrZ52Lohm0Bo9JwUAmAW_RwgX_qqBSGEyyyX3w8hx")
	else
		cb(false)
	end
end)
...
ESX.RegisterServerCallback('esx_vehicleshop:getCommercialVehicles', function (source, cb)
	MySQL.query('SELECT * FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function (result)
		local vehicles = {}
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.query('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerClientEvent('esx:Notify', source, 'info', _U('vehicle_sold_for', vehicleModel, ESX.Math.GroupDigits(price)))
		else
...
function PayRent(d, h, m)
	MySQL.query('SELECT * FROM rented_vehicles', {}, function (result)
		for i=1, #result, 1 do
			...
			if xPlayer ~= nil then
				...
				TriggerClientEvent('esx:Notify', xPlayer.source, 'info', _U('paid_rental', ESX.Math.GroupDigits(result[i].rent_price)))
			else -- pay rent either way
				MySQL.update('UPDATE users SET bank = bank - @bank WHERE identifier = @identifier',
				{
