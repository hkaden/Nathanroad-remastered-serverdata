				...
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.UI.Notify('info', _U('vehicle_purchased'))
						else
							ESX.UI.Notify('info', _U('broke_company'))
						end
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.UI.Notify('info', _U('not_enough_money'))
									end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elseif action == 'create_bill' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				ESX.UI.Notify('info', _U('no_players'))
				return
			end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					ESX.UI.Notify('info', _U('invalid_amount'))
				else
					menu2.close()
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				if Config.EnableOwnedVehicles then
					...
					ESX.UI.Notify('info', _U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
				else
					ESX.UI.Notify('info', _U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
				end
			end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						ESX.UI.Notify('info', _U('vehicle_set_rented', vehicleProps.plate, GetPlayerName(closestPlayer)))
...
RegisterNetEvent("esx_vehicleshop:OpenMobileActionsMenu")
AddEventHandler("esx_vehicleshop:OpenMobileActionsMenu", function()
	OpenMobileVSActionsMenu()
end)

function OpenMobileVSActionsMenu()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_vs_actions', {
		title    = _U('car_dealer'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'billing' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer == -1 or closestDistance > 3.0 then
						ESX.UI.Notify('info', _U('no_players_near'))
					else
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', 'Concessionnaire', amount)
						ESX.UI.Notify('info', _U('billing_sent'))
					end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			ESX.UI.Notify('info', _U('search_database_error_invalid'))
		else
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					ESX.UI.Notify('info', _U('search_database_found', owner))
				else
					ESX.UI.Notify('info', _U('search_database_error_not_found'))
				end
			end, data.value)
			menu.close()
		end
...
-- 	while true do
-- 		Citizen.Wait(0)
-- 		if IsControlJustReleased(0, 167) and IsInputDisabled(0) and not IsDead and Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.name == 'cardealer' then
-- 			OpenMobileVSActionsMenu()
-- 		end
-- 	end
-- end)
	...
	ESX.TriggerServerCallback('esx_vehicleshop:getStockItems', function (items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					ESX.UI.Notify('info', _U('quantity_invalid'))
				else
...
-- 								OpenShopMenu()
-- 							else
-- 								ESX.UI.Notify('info', _U('license_missing'))
-- 							end
...
-- 							ESX.UI.Notify('info', _U('delivered'))
-- 						else
-- 							ESX.UI.Notify('info', _U('not_rental'))
-- 						end
...
-- 						if vehicleSold then
-- 							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
-- 							ESX.UI.Notify('info', _U('vehicle_sold_for', CurrentActionData.label, ESX.Math.GroupDigits(CurrentActionData.price)))
-- 						else
-- 							ESX.UI.Notify('info', _U('not_yours'))
-- 						end
...
AddEventHandler("qs-luckywheel:winCar", function() 
    ...
    ESX.Game.SpawnVehicle("emerus", { x = 933.29 , y = -2.82 , z = 78.76 }, 144.6, function (vehicle) -- Vehicle name, emerus default vehicle.
        ...
        TriggerServerEvent('esx_vehicleshop:setVehicleOwned', vehicleProps, model)
