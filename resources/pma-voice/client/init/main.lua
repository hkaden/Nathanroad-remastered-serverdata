    ...
    while true do
        ...
        for id = 0, 256 do
            if  ((NetworkIsPlayerActive(id)) and GetPlayerPed(id) ~= PlayerPedId()) then
                ped = GetPlayerPed(id)
                selfCoords = GetEntityCoords(PlayerPedId(), true)
                targetCoords = GetEntityCoords(GetPlayerPed(id), true)
                distance = #(selfCoords - targetCoords) -- math.floor(GetDistanceBetweenCoords(x1, y1, z1, x2, y2, z2, true))
				local takeaway = 0.95
                if ((distance < 10) and IsEntityVisible(GetPlayerPed(id))) ~= PlayerPedId() then
				if NetworkIsPlayerTalking(id) then
					sleep = 3
					DrawMarker(0, targetCoords.x, targetCoords.y, targetCoords.z + 1.1 , 0, 0, 10, 0, 0, 0, 0.1, 0.1, 0.1, 55, 160, 205, 105, 1, 1, 2, 0, 0, 0, 0)
				else
                    end
                end  
            end
