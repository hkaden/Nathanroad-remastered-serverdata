...
Config = {
    ...
    Police = {
        Jobs            = {'police'},       -- police job names in database
	...
	Lock = {
		...
		UnlockInt  		= 1,						-- select between 1 or 0 for unlocked status. 
		...
		Chance			= 10,						-- % chance to enter moving vehicles
	...
	CarMenu = {
		Key				= 0,          		    -- key to open car menu (DEFAULT: [F3]), set to 0 to disable.
		Command 		= 'carmenu',				-- command to open car menu, set to '' to disable
	},

	Engine = {
		Key				= 0,          		    -- key to toggle engine on/off (DEFAULT: [K]), set to 0 to disable.
    ...
    Steal = {
        ...
        AllowSearch     = false,                    -- allow player to /search vehicle for goods if got keys? 
    ...
    Lockpick = {
        ...
        Command         = '',                   -- command to lockpick vehicle, set to '' to disable.
        Text            = Lang['progbar_lockpicking'],	-- string to use for e.g. progressBars?
        Duration        = 40000,							-- time in MS to lockpick vehicle
        Remove          = true, 						-- set to false to not remove the item on use.
        Chance          = 30,    						-- % chance to lockpick vehicles without alarms
        SetHotwire      = true,                         -- require player to hotwire the lockpicked vehicle, before he can drive.
        AllowSearch     = false,                         -- allow player to /search vehicle for goods?
        Anim = {
        ...
        Alarm = {
            ...
            Time        = 40000,						-- alarm sound duration in MS
            Chance      = 100,							-- % chance to lockpick owned vehicles equipped with an alarm
    ...
    Search = {
        ...
        Duration        = 20000,                     	-- duration for search in MS
        AnimDict        = 'veh@handler@base',       	-- anim dict
        AnimLib         = 'hotwire',                	-- anim lib
        Money = {
            Chance      = 50,                       	-- chance of finding money
            MinAmount   = 1000,                      	-- min amount of money
            MaxAmount   = 3000,                      	-- max amount of money
            BlackMoney  = false,                     	-- receive dirty cash, else normal cash.
        },
        -- Insert item, label, chance and min/max amounts (script loops through all items listed):
        Items = {
            [1] = {item = 'phone', name = '手機', chance = 67, amount = {min = 2, max = 4}},
        },
    },
    ...
    Hotwire = {
        ...
        Duration        = 30000,                     	-- duration for hotwire in MS
        AnimDict        = 'veh@handler@base',       	-- anim dict
        AnimLib         = 'hotwire',                	-- anim lib
        Chance          = 30,                       	-- % chance of successfull hotwiring
		Alarm = {
            Enable      = true,							-- enable car alarm sound & effects upon lockpicking
            Time        = 30000,						-- alarm sound duration in MS
            Chance      = 100,							-- % chance to lockpick owned vehicles equipped with an alarm
			Report		= true							-- report to owner/player of vehicle if vehicle has alarm?
        },
		Report 			= true 							-- report lockpicking to police?
...
Config.WhitelistCars = {
    [1] = {model = `bmw5pol`, job = {"police"}},
    [3] = {model = `bmwpoliceb`, job = {"police"}},
    [4] = {model = `polchiron`, job = {"police"}},
    [5] = {model = `policebenz`, job = {"police"}},
    [6] = {model = `rmodfordpolice`, job = {"police"}},
    [7] = {model = `benson`, job = {"chickenman"}},
    [8] = {model = `youga2`, job = {"tailor"}},
    [9] = {model = `flatbed`, job = {"mechanic"}},
    [10] = {model = `towtruck2`, job = {"mechanic"}},
    [11] = {model = `taxi`, job = {"taxi"}},
    [12] = {model = `polmav`, job = {"police","ambulance"}},
    [13] = {model = `hwp2018v2`, job = {"police"}},
    [14] = {model = `polgt2rs`, job = {"police"}},
    [15] = {model = `polbmwm4`, job = {"police"}},
    [16] = {model = `suppressor`, job = {"police"}},
    [17] = {model = `wmfenyrcop`, job = {"police"}},
    [18] = {model = `rumpo`, job = {"reporter"}},
}

Config.LockSmith = {
    pos = vector3(119.13, -140.36, 54.85),   -- posto open menu
    key = 38,                               -- key to open menu
    text = Lang['draw_locksmith'],          -- draw text 

    price = 1000,                            -- price to register a key
    bank = true,                            -- pay in bank money, false is normal cash

    marker = { enable = false, drawDist = 10.0, type = 20, scale = vector3(0.5,0.5,0.5), color = {r = 240, g = 52, b = 52, a = 100} },
    blip = { enable = false, sprite = 134, color = 1, label = Lang['blip_locksmith'], scale = 0.8, display = 4 },
}

Config.AlarmShop = {
    pos = vector3(-180.23, -1171.5, 23.77),  -- pos to open menu
    key = 38,                               -- key to open menu
