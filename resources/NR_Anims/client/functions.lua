...
Play.Animation = function(dance, particle, prop, p)
    if dance then
        ...
        TaskPlayAnim(PlayerPed, dance.dict, dance.anim, 1.5, 1.5, loop, move, 0, false, false, false)
...
Play.Scene = function(scene, p)
    if scene then
        ...
        if not scene.sex == 'both' and not (sex == scene.sex) then
            Play.Notification('info', '你的性別不允許這個動作')
        else
            if scene.sex == 'position' then
                local coords = GetOffsetFromEntityInWorldCoords(PlayerPed, 0.0, 0 - 0.5, -0.5);
                TaskStartScenarioAtPosition(PlayerPed, scene.scene, coords.x, coords.y, coords.z, GetEntityHeading(PlayerPed), 0, 1, false)
            else
                TaskStartScenarioInPlace(PlayerPed, scene.scene, 0, true)
            end
...
Play.Expression = function(expression, p)
    if expression then
        local PlayerPed = cache.ped
        SetFacialIdleAnimOverride(PlayerPed, expression.expressions, 0)
        p:resolve({passed = true})
        return
    end
    p:reject({passed = false})
...
Play.Walk = function(walks, p)
    if walks then
        ...
        SetPedMovementClipset(PlayerPed, walks.style, cfg.walkingTransition)
...
Play.Prop = function(props)
    if props then
        ...
        if props.prop then
            ...
            Load.PropCreation(PlayerPed, props.prop, props.propBone, props.propPlacement)
        end
        if props.propTwo then
            Load.Model(props.propTwo)
            Load.PropCreation(PlayerPed, props.propTwo, props.propTwoBone, props.propTwoPlacement)
        end
    end
end
...
Play.Ptfx = function(ped, particles)
    if particles then
        ...
        if not attachedProp and not cfg.ptfxEntitiesTwo[NetworkGetEntityOwner(ped)] and not cfg.ptfxOwner and ped == PlayerPed then
...
Play.Shared = function(shared, p)
    if shared then
        ...
        if closePed then
            ...
            Play.Notification('info', '已發送請求給 ' .. GetPlayerName(targetId))
...
RegisterNetEvent('anims:requestShared', function(shared, targetId, owner)
    if type(shared) == "table" and targetId then
        ...
        if targetPlayer then
            ...
            Load.Dict(shared[1])
            TaskPlayAnim(ped, shared[1], shared[2], 2.0, 2.0, shared[4] or 3000, 1, 0, false, false, false)
