...
RegisterNetEvent('esx:setJob', function(Job)
    ...
    TriggerServerEvent('NR_Customs:server:getUsingStatus')
end)
...
--#[Global Functions]#--
function AttemptPurchase(type, price, model, plate, upgradeLevel)
    if upgradeLevel ~= nil then
        upgradeLevel = upgradeLevel + 2
    end
    TriggerServerEvent("qb-customs:attemptPurchase", type, price, model, plate, upgradeLevel)
...
function ExitBennys(k)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
    TriggerServerEvent('NR_Customs:server:updateUsingStatus', false, k)
    saveVehicle()
...
RegisterNetEvent('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        if not isPlyInBennys then -- Bennys
            enterLocation(bennyLocation, useID)
        end
    end
end)

RegisterNetEvent('event:control:bennysAdmin', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        -- bennyHeading = bennyGarages[useID].coords.w
        if not isPlyInBennys then -- Bennys
            openAdminCustom()
...
function disableControlsAdmin(k)
    Wait(1000)
    CreateThread(function()
        while isPlyInBennys do
            ...
            if IsDisabledControlJustPressed(1, 176) then --Key: Enter
                MenuManager(true, k)
                PlaySoundFrontend(-1, "OK", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
            end
        
            if IsDisabledControlJustPressed(1, 177) then --Key: Backspace
                MenuManager(false, k)
...
function openAdminCustom()
    ...
    isPlyInBennys = true
    disableControlsAdmin('admin')
end
exports('openAdminCustom', openAdminCustom)

function enterLocation(locationsPos, useID)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
    local isMotorcycle = false
    ...
    isPlyInBennys = true
    disableControls(useID)
end

function disableControls(k)
    Wait(1000)
    CreateThread(function()
        while isPlyInBennys do
            DisableControlAction(1, 38, true) --Key: E
        ...
        if ESX.PlayerLoaded then
            ...
            if ESX.PlayerData.job.name == "mechanic" then
                if IsPedInAnyVehicle(plyPed, false) then
                    ...
                    for k, v in pairs(bennyGarages) do
                        nearDefault = isNear(plyPos, vector3(v.coords.x,v.coords.y,v.coords.z), 2.5)
                        if nearDefault then
                            ...
                            if nearDefault then
                                ...
                                if not isPlyInBennys then
                                    Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[按 ~p~E~w~ - 進入改裝店]", 255, 255, 255, 255, 1, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                                    if IsControlJustReleased(1, 38) then
                                        if GetPedInVehicleSeat(GetVehiclePedIsIn(plyPed), -1) == plyPed then
                                            -- print(bennyGarages[k].using, 'bennyGarages[k].using')
                                            if (v.useJob and isAuthorized((ESX.PlayerData.job.name), k, ESX.PlayerData.job.grade)) or not v.useJob then
                                                if not bennyGarages[k].using then
                                                    TriggerEvent('event:control:bennys', k)
                                                    -- print(k , 'k')
                                                    TriggerServerEvent('NR_Customs:server:updateUsingStatus', true, k)
                                                else
                                                    ESX.UI.Notify('error', '有人正在使用中')
                                                end
                                            else
                                                ESX.UI.Notify("error", "你不能使用")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent("qb-customs:purchaseSuccessful", function()
    ...
    ESX.UI.Notify("success", "成功購買")
end)

RegisterNetEvent("qb-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    ESX.UI.Notify("error", "沒有足夠金錢")
end)

RegisterNetEvent("NR_Customs:client:updateUsingStatus", function(status, k)
    bennyGarages[k].using = status
end)

RegisterNetEvent("NR_Customs:client:getUsingStatus", function(status)
    bennyGarages = status
end)

--helper function

function isAuthorized(job, location, grade)
    if grade >= bennyGarages[location].job[job] then
        return true
    end
    return false
end
