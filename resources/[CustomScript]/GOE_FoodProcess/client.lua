		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'How_much', {title = "How much do you want to sell?"}, 
		  function(data, menu)
			...
			if amount == nil or amount < 1 then
				...
				TriggerServerEvent("NR_FoodProcess:sellfixkit", 'fixkit', amount)
				menu.close()
			end
		end, 
		function(data, menu)
...
function SellEndurDialogue()
	...
	function(data, menu)
	  ...
	  if amount == nil or amount < 1 then
		  ...
		  TriggerServerEvent("NR_FoodProcess:sellendur", 'endur_nitro', amount)
		  menu.close()
	  end
	end, 
	function(data, menu)
...
function SellPowerDialogue()
	...
	function(data, menu)
	  ...
	  if amount == nil or amount < 1 then
		  ...
		  TriggerServerEvent("NR_FoodProcess:sellpower", 'power_nitro', amount)
		  menu.close()
	  end
	end, 
	function(data, menu)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'unicorn' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('NR_FoodProcess:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('NR_FoodProcess:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
...
AddEventHandler('NR_FoodProcess:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('NR_FoodProcess:hasEnteredMarker', function(station, part, partNum)
	if part == 'MakeItem' then
		CurrentAction     = 'menu_make'
		CurrentActionMsg  = '按 ~INPUT_CONTEXT~ 加工食品'
		CurrentActionData = {}
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'unicorn' then
...
function OpenFoodProcessingMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_harvest', {
	  ...
	  ESX.TriggerServerCallback('NR_FoodProcess:IsItOver' , function(itemover)
		if not itemover then
		  ...
		  TriggerEvent("pNotify:SendNotification",{text = "你沒有足夠的空間",type = "warning",timeout = (4000),layout = "centerRight",queue = "global"})
			...
			ESX.TriggerServerCallback('NR_FoodProcess:getPlayerInventory' , function(dataitem)
			  for i=1, #dataitem.items do
				...
				if data.current.type == 'recipe' then
				  ESX.TriggerServerCallback('NR_FoodProcess:getPlayerInventory' , function(dataitemrecipe)
...
function startProcessing(itemdata)
	if not isBusy then
		isBusy = true 
		TriggerEvent('MF_LockPicking:StartMinigame','NR_FoodProcess:succesChickenSteak',itemdata,Config.Notitext)
    end
end
...
RegisterNetEvent('NR_FoodProcess:setisBusyToFalse')
AddEventHandler('NR_FoodProcess:setisBusyToFalse', function() setBusyToFalse() end)

RegisterNetEvent('NR_FoodProcess:animation')
AddEventHandler('NR_FoodProcess:animation', function(itemdata) animation(itemdata); end)
