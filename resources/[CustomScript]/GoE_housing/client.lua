                    ...
                    if IsControlJustReleased(0, 38) then
                        if OwnedHouse.houseId == k then
                                ...
                                function(data, menu)
                                    ...
                                    elseif data.current.value == 'garage' then
                                        ...
                                        if found then
                                            ...
                                            ESX.UI.Notify('info', Strings['No_Spawn'])
                                        end
                            ...
                            if not AvailableHouses[k] and OwnedHouse.houseId == 0 then
                                ...
                                {
                                    title = (Strings['Buy_House_Confirm']):format(v['price'], k),
                                    align = 'top-left',
                                    elements = {
                                        {label = Strings['yes'], value = 'yes'},
                                        {label = Strings['no'], value = 'no'}
        ...
        if dist <= 50.0 then
            ...
            if dist <= 1.5 then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if IsModelValid(model) then
                        ...
                        while not HasModelLoaded(model) do 
                            ...
                            if GetGameTimer() - startedLoading >= 1500 then
                                ESX.UI.Notify('info', ('The model (%s) is taking a looong time to load. Contact the server owner.'):format(Config.Furniture[category[1]][current][2]))
                                break
                            end
                        end
                        ...
                        ESX.UI.Notify('info', ('The model (%s) isn\'t valid. Contact the server owner.'):format(Config.Furniture[category[1]][current][2]))
                    end
                    ...
                    while true do
                        Wait(0)
                        HelpText((Strings['Buying_Furniture']):format(category[2], Config.Furniture[category[1]][current][3], Config.Furniture[category[1]][current][1]))
                        ...
                        elseif IsControlJustReleased(0, 191) then
                            ...
                            {
                                title = (Strings['Confirm_Purchase']):format(Config.Furniture[category[1]][current][3], Config.Furniture[category[1]][current][1]),
                                align = 'center',
                                elements = {
                                    {label = Strings['yes'], value = 'yes'},
                                    {label = Strings['no'], value = 'no'}
        ...
        if Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
            ...
            if IsControlJustReleased(0, 38) and Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
                ...
                function(data, menu)
                    ...
                    elseif data.current.value == 'blackmoney' then
                        blackMoneyStorage(OwnedHouse.houseId)
                    end
                end, 
                    function(data, menu)
                    menu.close()
                    ...
                    if data.current.value == 'exit' then
                        ESX.TriggerServerCallback('loaf_housing:hasGuests', function(has)
                            ...
                            if not has then
                                ...
                                ESX.UI.Notify('info', Strings['Guests'])
                            end
                        end)
...
AddEventHandler('loaf_housing:knockAccept', function(coords, house, storage, spawnpos, furniture, host)
    ...
    while true do
        ...
        if delete then 
            ...
            DeleteObject(prop)
            DoScreenFadeIn(1500)
            ESX.UI.Notify('info', Strings['Host_Left'])
            return
        end
...
AddEventHandler('loaf_housing:knockedDoor', function(src)
    ESX.UI.Notify('info', Strings['Someone_Knocked'])
