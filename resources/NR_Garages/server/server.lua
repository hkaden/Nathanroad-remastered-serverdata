...
if Config.RestoreVehicles then
    MySQL.ready(function()
        MySQL.update("UPDATE `owned_vehicles` SET `stored` = 1, `garage` = `last_garage` WHERE `stored` = 0 AND `garage` <> 'HOUSE'")
    end)
end
...
lib.callback.register('luke_garages:GetVehicles', function(source, garageType, job)
    ...
    if not job then
        local results = MySQL.query.await("SELECT `plate`, `vehicle`, `stored`, `garage`, `job` FROM `owned_vehicles` WHERE `owner` = @identifier AND `type` = @type", {
        ...
        local results = MySQL.query.await('SELECT `plate`, `vehicle`, `stored`, `garage` FROM `owned_vehicles` WHERE (`owner` = @identifier OR `owner` IN (@jobs)) AND `type` = @type AND `job` IN (@jobs)', {
            ['@identifier'] = identifier,
            ['@type'] = garageType,
            ['@jobs'] = jobs
        })
...
lib.addCommand('group.admin', 'vehs', function(source, args)
	tprint(GetAllVehicles())
end, {})
...
lib.callback.register('luke_garages:GetImpound', function(source, type)
    ...
    local results = MySQL.query.await('SELECT `plate`, `vehicle`, `job` FROM owned_vehicles WHERE (`owner` = @identifier OR `owner` = @job) AND `type` = @type AND `stored` = 0', {
        ['@identifier'] = identifier,
        ['@type'] = type,
...
lib.callback.register('luke_garages:CheckOwnership', function(source, plate, model, currentGarage)
    ...
    local jobs = {}
    if type(job) == 'table' then for k, _ in pairs(job) do jobs[#jobs+1] = k end else jobs = job end
    local result = MySQL.query.await("SELECT `vehicle`, `job`, `type` FROM owned_vehicles WHERE (`owner` = @owner OR `job` IN (@jobs)) AND `plate` = @plate", {
        ['@owner'] = identifier,
        ['@plate'] = ESX.Math.Trim(plate),
        ['@jobs'] = jobs
    })
...
RegisterNetEvent('luke_garages:ChangeStored', function(plate)
    ...
    MySQL.update('UPDATE `owned_vehicles` SET `stored` = @stored, `garage` = @garage WHERE `plate` = @plate', {
        ['@garage'] = 'none',
        ['@stored'] = 0,
        ['@plate'] = plate
    })
...
RegisterNetEvent('luke_garages:SaveVehicle', function(vehicle, plate, ent, garage)
    MySQL.update('UPDATE `owned_vehicles` SET `vehicle` = @vehicle, `garage` = @garage, `last_garage` = @garage, `stored` = @stored WHERE `plate` = @plate', {
