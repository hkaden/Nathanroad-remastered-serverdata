...
RegisterNetEvent('ox_doorlock:setState', function(id, state, source, data)
	if not doors then return print(not doors) end
	...
	if double then
		if not door.auto and door.state == 1 then
			print(door.auto, door.state, double[1].hash, double[1].entity, double[2].hash, double[2].entity, 'door.auto, door.state, double[1].hash, double[1].entity, double[2].hash, double[2].entity')
			while double[1].entity and double[2].entity do
				if door.state ~= 1 then return print('door.state ~= 1') end
				...
				print(doorOneHeading, doorTwoHeading, doorOneCurrentHeading, doorTwoCurrentHeading, 'doorOneHeading, doorOneCurrentHeading')

				if doorOneHeading == doorOneCurrentHeading and doorTwoHeading == doorTwoCurrentHeading or (doorOneHeading == 0 or doorTwoHeading == 0) then print('double break') break end
				Wait(0)
			end
		end
		...
		print(door.auto, door.state, door.hash, door.entity, 'door.auto, door.state, door.hash, door.entity')
		if not door.auto and door.state == 1 then
			while door.entity do
				if door.state ~= 1 then return print('door.state ~= 1') end

				local heading = math.floor(GetEntityHeading(door.entity) + 0.5)
				print(door.entity, GetEntityHeading(door.entity), heading, door.heading, 'heading, door.heading')
				if heading == door.heading or heading == 0 then print('double break') break end
				Wait(0)
			end
		end
