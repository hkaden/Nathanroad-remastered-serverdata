...
function closeMenu() {
    $.post("https://NR_Crafting/close", JSON.stringify({}));
        ...
        for (const [key, value] of Object.entries(recipes)) {
            if (value.Category == key1) {
                if (value.Level > level) {
                    ...
                    if (
                        value.requireBlueprint &&
                        (inventory[key + "_blueprint"] == 0 ||
                            inventory[key + "_blueprint"] == null)
                    ) {
                        if (!hidecraft) {
                            add = true;
                        }
                        ...
                        if (
                            value.Jobs.includes(job) ||
                            Object.keys(value.Jobs).length == 0
                        ) {
                            if (
                                value.JobGrades.includes(grade) ||
                                Object.keys(value.JobGrades).length == 0
                            ) {
                                add = true;
                            } else {
                                if (!hidecraft) {
        ...
        if (value.Category == category) {
            if (value.Level > level) {
                if (!hidecraft) {
                    second =
                        second +
                        '    <div class="clearfix colelem recipe" onclick="inspect(this)" data-item="' +
                        key +
                ...
                if (
                    ...
                    if (!hidecraft) {
                        second =
                            ...
                            key +
                            '" onclick="inspect(this)" style="opacity: 0.5;" id="pu212"><!-- group -->' +
                            '     <div class="gradient grpelem" id="u212" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu212"><!-- simple frame --></div>' +
                            '     <div class="clearfix grpelem" id="u225-4" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu212"><!-- content -->' +
                            "      <p>" +
                            String(names[key]).toUpperCase() +
...
function inspect(t) {
    if (opened != t) {
        ...
        var base =
            ...
            '    <div class="museBGSize grpelem" id="u389" style="background: url(img/' + t.dataset.item +
            ...
            '     <button class="ripple" id="u407-4" data-item="' + t.dataset.item +
            '"  data-sizePolicy="fixed" data-pintopage="page_fixedCenter"><!-- content -->' + "      <p>等級不足</p>" + "     </button>" +
            ...
            "     <p>" + String(names[t.dataset.item]).toUpperCase() + "</p>" + "    </div>" +
            ...
            "     <p>材料</p>" + "    </div>" + '<div id="ingredients">';
        if ((grade >= item.JobGrades || item.JobGrades.length <= 0)) {
            if (level >= item.Level) {
                base =
                    '   <div class="slide-bottom " id="pu386"><!-- group -->' +
                    '    <div class="gradient grpelem" id="u386" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu386"><!-- simple frame --></div>' +
                    ...
                    '.png) no-repeat center; background-size: 120%; " data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu386"><!-- simple frame --></div>' +
                    '     <button class="ripple" id="u407-4" data-item="' + t.dataset.item +
                    '" onclick="craft(this)" data-sizePolicy="fixed" data-pintopage="page_fixedCenter"><!-- content -->' + "      <p>合成</p>" + "     </button>" +
                    '    <div class="clearfix grpelem" id="u457-4" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu386"><!-- content -->' +
                    "     <p>" + String(names[t.dataset.item]).toUpperCase() + "</p>" + "    </div>" +
                    '    <div class="rounded-corners clearfix grpelem" id="u535-4" data-sizePolicy="fixed" data-pintopage="page_fixedCenter" data-leftAdjustmentDoneBy="pu386"><!-- content -->' +
                    "     <p>" + timeString + "</p>" + "    </div>" +
...
window.addEventListener("message", function (event) {
    ...
    if (edata.type == "crafting") {
        for (const [key, value] of Object.entries(
            recipes[edata.item].Ingredients
        )) {
            if (inventory[key] >= value) {
