            ...
            if ePos.x ~= 0.0 then
              ...
              if not furni.antiGrab[model] then
                if wasPressing and keyHeldFor >= 500 then
                  Utils.drawText3D(pos,"~r~放開 [~s~左鍵~r~] 修改 [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
                else
                  Utils.drawText3D(pos,"~y~按住 [~s~左鍵~y~] 修改 [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
                end
...
CanPlayerAfford = function(value)
  if GetPlayerCash() >= value then
    print('can afford? ' .. GetPlayerCash() .. ' ' .. value)
    return true
