...
CreateThread(function()
    while true do
        Wait(1000)
        if NetCheck1 then
            TriggerServerEvent('AdminMenu:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('AdminMenu:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        ESX.UI.Notify('success', '已啟用')
    else
        ShowBlips = false
        ESX.UI.Notify('error', '已停用')
    end
end)

RegisterNetEvent('AdminMenu:client:Show', function(players)
        ...
        if ShowBlips then
            if not DoesBlipExist(blip) then
                ...
                if IsPauseMenuActive() then
                    ...
                    local x1, y1 = table.unpack(GetEntityCoords(PlayerPed, true))
                    local x2, y2 = player.coords.x, player.coords.y
