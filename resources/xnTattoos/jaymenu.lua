...
local function setMenuProperty(id, property, value)
    if id and menus[id] then
        ...
        -- debugPrintid..' menu property changed: { '..tostring(property)..', '..tostring(value)..' }')
    end
end
...
function JayMenu.CreateMenu(id, title, closeCallback)
    ...
    -- debugPrinttostring(id)..' menu created')
end
...
function JayMenu.OpenMenu(id)
    if id and menus[id] then
        ...
        -- debugPrinttostring(id)..' menu opened')
    else
...
function JayMenu.CloseMenu()
    if menus[currentMenu] then
        if menus[currentMenu].aboutToBeClosed then
            ...
            setMenuVisible(currentMenu, false)
            -- debugPrinttostring(currentMenu)..' menu closed')
            ...
            if menus[currentMenu].closeCallback() then
                ...
                -- debugPrinttostring(currentMenu)..' menu about to be closed')
            end
        end
    end
end
    ...
    if menus[currentMenu] then
        ...
        if isCurrent then
			...
			elseif currentKey == keys.select then
				...
				-- debugPrintbuttonText..' button pressed')
...
function JayMenu.CheckBox(text, bool, callback)
    ...
    if JayMenu.SpriteButton(text, "commonmenu", sprite, focusSprite) then
        bool = not bool
        -- debugPrinttostring(text)..' checkbox changed to '..tostring(bool))
        callback(bool)
