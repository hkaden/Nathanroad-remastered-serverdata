...
function GetNaked()
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        if skin.model == 'mp_m_freemode_01' then
            ...
            exports['NR_Appearance']:setPedComponents(PlayerPedId(), {arms, shirt, torso, pants, shoes})
        else
            local arms = {
                component_id = 3,
                texture = 0,
...
function BuyTattoo(collection, name, label, price)
    ESX.TriggerServerCallback('SmallTattoos:PurchaseTattoo', function(success)
        if success then
            ...
            TriggerEvent('fivem-appearance:myskin')
        end
...
function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "訪問紋身店", 38)
		scaleString = sType
...
Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "紋身店", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "分類")
