-- https://modit.store
NR_Inventory = exports.NR_Inventory
...
VehicleStored = function(id,plate,props)
  MySQL.update("UPDATE owned_vehicles SET `stored`=false,`storedhouse`=@storedhouse WHERE `plate`=@plate",{['@storedhouse'] = id,['@plate'] = plate})
end
...
PurchaseHouse = function(house)
  ...
  local xPlayer = GetPlayerData(_source)
  local v = Houses[house.Id]
  ...
  if not v.Owned then
    ...
    if xPlayer.getMoney() >= v.Price then
      ...
      local whData = {
        message = xPlayer.identifier .. ", " .. xPlayer.name .. ", 以$, " .. v.Price .. ", 購買了, " .. house.Id,
        sourceIdentifier = xPlayer.identifier,
        event = 'allhousing:PurchaseHouse'
      }
      local additionalFields = {
          _type = 'Housing:Purchase:Cash',
    ...
    elseif xPlayer.getAccount('bank') >= v.Price then
      afford = true
      print('Testing2')
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          if targetPlayer then
            ...
            targetPlayer.addAccountMoney('bank', salePrice)
            NotifyPlayer(targetSource,string.format(Labels["HousePurchased"],v.Price,(v.Price ~= salePrice and Labels["HouseEarning"] or ".")))

            local whData = {
              message = targetPlayer.identifier .. ", " .. targetPlayer.name  .. ", 以$, " .. salePrice .. ", 出售了, " .. house.Id,
              sourceIdentifier = xPlayer.identifier,
              event = 'allhousing:PurchaseHouse'
            }
            local additionalFields = {
                _type = 'Housing:OnlineSell',
                _Seller = targetPlayer.identifier,
                _SellerName = targetPlayer.name,
                _Price = salePrice,
                _HouseID = house.Id
            }
            TriggerEvent('NR_graylog:createLog', whData, additionalFields)

            -- TriggerEvent('esx:sendToDiscord', 16753920, "購買公寓 - 出售", targetPlayer.identifier .. ", " .. targetPlayer.name  .. ", 以$, " .. salePrice .. ", 出售了, " .. house.Id .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/758351257852969020/crVfxPbFI3o1KwmpoG1GLOVJ0cciAZecI9fbwEL-Lb-83dnG6OJ0x25SfMkeE-X5yEaP")
          else
            AddOfflineBank(v.Owner,salePrice)
            local whData = {
              message = v.Owner .. ", 以$, " .. salePrice .. ", 出售了, " .. house.Id,
              sourceIdentifier = xPlayer.identifier,
              event = 'allhousing:PurchaseHouse'
            }
            local additionalFields = {
                _type = 'Housing:OfflineSell',
                _Seller = v.Owner,
                _Price = salePrice,
                _HouseID = house.Id
            }
            ...
            -- TriggerEvent('esx:sendToDiscord', 16753920, "購買公寓 - 離線出售", v.Owner .. ", 以$, " .. salePrice .. ", 出售了, " .. house.Id .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/758351257852969020/crVfxPbFI3o1KwmpoG1GLOVJ0cciAZecI9fbwEL-Lb-83dnG6OJ0x25SfMkeE-X5yEaP")
          end
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            ...
            if societyAmount and societyAccount then
              exports.NR_Banking:AddJobMoney(societyAccount, societyAmount)
            end
          end
        end
      end

      v.Owner = xPlayer.identifier
      v.OwnerName = xPlayer.name
      v.ResaleJob = ""
      v.Owned = true
      TriggerClientEvent("Allhousing:SyncHouse",-1,v)
      SqlExecute("UPDATE allhousing SET owner=@owner,ownername=@ownername,resalejob=@resalejob,owned=1,housekeys=@housekeys WHERE id=@id",{['@owner'] = xPlayer.identifier,['@ownername'] = v.OwnerName, ['@housekeys'] = json.encode({}),['@resalejob'] = "",['@id'] = v.Id})
    end
  end
end
...
SellHouse = function(house,price)
  ...
  if v.Entry.x == entry.x and v.Entry.y == entry.y and v.Entry.z == entry.z then
    if v.Owned and v.Owner == xPlayer.identifier then
          ...
          TriggerEvent("Allhousing.Furni:GetPrices", function(prices)
            ...
            if v and v.Furniture and type(v.Furniture) == "table" then
              ...
              if count and count > 0 then
                _print("[Sale]","Added $"..addVal.." to ".._source.." ("..xPlayer.identifier.."/"..xPlayer.name..") to refund "..(count and count > 1 and count.." pieces" or tostring(count).." piece").." of furniture.")

                xPlayer.addAccountMoney('bank', math.floor(addVal * (Config.RefundPercent / 100)))
                local whData = {
                  message = xPlayer.identifier .. ", " .. xPlayer.name .. ", 以$, " .. math.ceil(addVal*(Config.RefundPercent / 100)) .. ", 出售並退款傢私$, " .. (count and count > 1 and count .. " pieces" or tostring(count).." piece"),
                  sourceIdentifier = xPlayer.identifier,
                  event = 'allhousing:SellHouse'
                }
                local additionalFields = {
                    _type = 'Housing:SellHouse:Trigger',
                    _Seller = xPlayer.identifier,
                    _SellerName = xPlayer.name,
                    _Price = math.ceil(addVal*(Config.RefundPercent / 100)),
                    _HouseID = house.Id
                }
                TriggerEvent('NR_graylog:createLog', whData, additionalFields)

                -- TriggerEvent('esx:sendToDiscord', 16753920, "購買公寓 - 出售", xPlayer.identifier .. ", " .. xPlayer.name .. ", 以$, " .. math.ceil(addVal*(Config.RefundPercent / 100)) .. ", 出售並退款傢私$, " .. (count and count > 1 and count.." pieces" or tostring(count).." piece") .. ", " .. os.date("%Y/%m/%d, %H:%M:%S",os.time()), "", "https://discordapp.com/api/webhooks/758351257852969020/crVfxPbFI3o1KwmpoG1GLOVJ0cciAZecI9fbwEL-Lb-83dnG6OJ0x25SfMkeE-X5yEaP")
              end
            end
          end)
        end
...
MortgageHouse = function(house)
  ...
  if not v.Owned then
    ...
    elseif xPlayer.getAccount('bank') >= price then
      afford = true
      print('Testing3')
...
GetVehicleOwner = function(source,plate)
  local res,ret
  MySQL.query("SELECT * FROM owned_vehicles WHERE plate=@plate",{['@plate'] = plate},function(retData)
    ret = {}
...
Citizen.CreateThread(function()
  local stashreg
  MySQL.query('SELECT * FROM `allhousing`', {}, function(result)
      properties = result
      for i=1, #properties, 1 do
        stashreg = properties[i]
        local stash = {
          id = "allhousing-"..stashreg.id,
          label = stashreg.id,
          slots = 50,
          weight = 300000,
          owner = false,
        }
        NR_Inventory:RegisterStash(stash.id, stash.label, stash.slots, stash.weight, stash.id)
      end
  end)
end)
...
RepayMortgage = function(id,repay)
  ...
  if house.Owned and house.MortgageOwed >= 0 then
    ...
    local afford
    if xPlayer.getMoney() >= repay then
      afford = true
      xPlayer.removeMoney(repay)
    elseif xPlayer.getAccount('bank') >= repay then
      afford = true
      print('Testing1')
      xPlayer.removeAccountMoney('bank',repay)
    end
    if afford then
