...
OpenInventory = function()
  if Config.UsingESX then
    if not Config.UsingDiscInventory then
      print('OpenInventory', InsideHouse.Id)
      exports.NR_Inventory:openInventory('stash', {id="allhousing-"..InsideHouse.Id, owner = GetPlayerData().identifier})
...
UpgradeHouse = function(d,data)
  if CanPlayerAfford(ShellPrices[data.shell]) then
    ...
    local whData = {
      message = GetPlayerData().identifier .. ", 為, " .. InsideHouse.Id .. ", 升級了, " .. data.shell .. ",價值$, " .. ShellPrices[data.shell],
      sourceIdentifier = GetPlayerData().identifier,
      event = 'UpgradeHouse'
    }
    local additionalFields = {
        _type = 'Housing:UpgradeHouse',
        _playerName = GetPlayerName(PlayerId()),
        _HouseID = InsideHouse.Id,
        _Shell = data.shell,
        _ShellPrice = ShellPrices[data.shell]
    }
    TriggerEvent('NR_graylog:createLog', whData, additionalFields)
    -- TriggerServerEvent('esx:sendToDiscord', 16753920, "公寓 Upgrade", GetPlayerData().identifier .. ", 為, " .. InsideHouse.Id .. ", 升級了, " .. data.shell .. ",價值$, " .. ShellPrices[data.shell], "", "https://discord.com/api/webhooks/735833202073141348/WfFtLHdk5Lqgbh9F7g5LoypUSy-7DGCBSfVs_z-Zm6KLsQBwcPRZqJuqdiqiwEtFuV0k")
