						...
						if data then
							if data.name == currentWeapon.ammo then
								...
								if not cache.vehicle then
									exports['BasicScript']:toggleIsBusy(true)
								...
								if cache.vehicle then
									...
									Wait(1500)
									exports['BasicScript']:toggleIsBusy(false)
								end
							end
						end
					end)
...
local function openNearbyInventory()
	if canOpenInventory() then
		local _, targetId, targetPed = Utils.GetClosestPlayer()
			...
			if invBusy then
				...
				elseif cache.vehicle then
					...
					if NetworkGetEntityIsNetworked(vehicle) then
						...
						client.openInventory('glovebox', {id = 'glove'..plate, class = vehicleClass, model = vehicleHash, plate = plate, netid = NetworkGetNetworkIdFromEntity(vehicle) })
					...
					if #(playerCoords - position) < 6 and NetworkGetEntityIsNetworked(vehicle) then
						...
						if locked == 0 or locked == 1 then
							...
							if closeToVehicle then
								...
								lastVehicle = vehicle
								client.openInventory('trunk', {id='trunk'..plate, class = vehicleClass, model = vehicleHash, plate = plate, netid = NetworkGetNetworkIdFromEntity(vehicle)})
								local timeout = 20
								repeat Wait(50)
									timeout -= 1
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player, source)
	...
	SendNUIMessage({
		...
		data = {
			...
			imagepath = GetConvar('inventory:imagepath', 'nui://nr_inventory/web/images')
		}
	})
