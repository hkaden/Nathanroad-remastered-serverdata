...
Item('akm_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_AKM', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
		end
...
Item('m9a3_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_M9A3', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
...
Item('carbine_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_CARBINERIFLE', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
...
Item('doubleaction_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_DOUBLEACTION', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
...
Item('heavy_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_HEAVYPISTOL', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
...
Item('stunshotgun_repair', function(event, item, inventory, slot, data)
	if event == 'usingItem' then
		if Inventory.GetItem(inventory, item, inventory.items[slot].metadata, true) > 0 then
			Inventory.AddItem(inventory.id, 'WEAPON_STUNSHOTGUN', 1, inventory.items[slot].metadata)
			TriggerClientEvent('ox_inventory:notify', inventory.id, {text = item.server.msg})
			return {
				inventory.label, event, 'external item use poggies'
			}
