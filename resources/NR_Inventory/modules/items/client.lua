...
Item('cuffs', function(data, slot)
	local targetDistance, targetId, targetPed = client.utils.GetClosestPlayer()
	if targetDistance <= 2.0 then
		ox_inventory:useItem(data, function(data)
			if data and targetId ~= 0 then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(targetId), true)
			end
		end)
	end
end)

-- Item('rope', function(data, slot)
-- 	local targetDistance, targetId, targetPed = client.utils.GetClosestPlayer()
-- 	if targetDistance <= 2.0 then
-- 		ox_inventory:useItem(data, function(data)
-- 			if data and targetId ~= 0 then
-- 				TriggerServerEvent('esx_policejob:handcuff_rope', GetPlayerServerId(targetId), true)
-- 			end
-- 		end)
-- 	end
-- end)

Item('cuff_keys', function(data, slot)
	local targetDistance, targetId, targetPed = client.utils.GetClosestPlayer()
	if targetDistance <= 2.0 then
		ox_inventory:useItem(data, function(data)
			if data and targetId ~= 0 then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(targetId), false)
			end
		end)
	end
end)

Item('identification', function(data, slot)
	TriggerEvent('qidentification:identification', slot)
end)

Item('drivers_license', function(data, slot)
	TriggerEvent('qidentification:drivers_license', slot)
end)

Item('firearms_license', function(data, slot)
	TriggerEvent('qidentification:firearms_license', slot)
end)

Item('burgershot_strangecandy', function(data, slot)
	ox_inventory:useItem(data, function(data)
		if data then
			TriggerEvent('NR_Restaurants:client:CandyCandy')
		end
	end)
end)

Item('mooncake', function(data, slot)
	ox_inventory:useItem(data, function(data)
		if data then
			exports['NR_Alert']:SendMidMessages()
		end
	end)
end)
