		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				if canAfford then
					...
					local whData = {
						message = playerInv.label .. ' [' .. playerInv.id .. '] - ' .. playerInv.owner .. ' Message: ' .. message,
						sourceIdentifier = playerInv.owner,
						event = 'Inventory:buyItem'
					}
					local additionalFields = {
						_type = 'Inventory:buyItem',
						_playerLabel = playerInv.label, -- GetPlayerName(PlayerId())
						_playerID = playerInv.id,
						_playerOwner = playerInv.owner,
						_count = count,
						_item = fromItem.label,
						_currency = Items(currency).label,
						_price = price
					}
					if metadata.serial then
						whData = {
							message = playerInv.label .. ' [' .. playerInv.id .. '] - ' .. playerInv.owner .. ' Message: ' .. message .. ' Metadata Serial: ' .. metadata.serial,
							sourceIdentifier = playerInv.owner,
							event = 'Inventory:buyItem'
						}
						additionalFields = {
