		...
		if inv then
			...
			if toSlot then
				if type(toSlot) == 'number' then
					...
					if inv.type == 'player' then
						local whData = {
							message = inv.owner .. ", " .. inv.label .. ", 添加了, x" .. count .. ", " .. item.name .. ", " .. item.label,
							sourceIdentifier = inv.owner,
							event = 'Inventory:func:AddItem'
						}
						local additionalFields = {
							_type = 'Inventory:AddItem',
							_player_name = inv.label,
							_item = item.name,
...
function Inventory.RemoveItem(inv, item, count, metadata, slot)
	...
	if item and count > 0 then
		...
		if removed > 0 and inv.type == 'player' then
			...
			local whData = {
				message = inv.owner .. ", " .. inv.label .. ", 移除了, x" .. removed .. ", " .. item.name .. ", " .. item.label,
				sourceIdentifier = inv.owner,
				event = 'Inventory:func:RemoveItem'
			}
			local additionalFields = {
				_type = 'Inventory:RemoveItem',
				_player_name = inv.label,
				_item = item.name,
				_item_label = item.label,
				_count = removed,
...
local function dropItem(source, data)
	...
	local whData = {
		message = playerInventory.owner .. ' [' .. playerInventory.id .. '] - ' .. playerInventory.label .. ' Droped ' .. data.count .. 'x ' .. toData.name .. ' to ' .. dropId,
		sourceIdentifier = playerInventory.owner,
		event = 'Inventory:CreateDrop'
	}
	local additionalFields = {
		_type = 'Inventory:CreateDrop',
		_playerInventoryLabel = playerInventory.label, -- GetPlayerName(PlayerId())
		_playerInventoryID = playerInventory.id,
		_playerInventoryOwner = playerInventory.owner,
		_count = data.count,
		_item = toData.name,
		_coords = coords,
		_dropID = dropId
					...
					if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
						...
						if not sameInventory then
							if fromInventory.type == 'container' or (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
								...
								local whData = {
									message = fromInventory.label .. ' [' .. fromInventory.id .. '] ' .. ' Swapped ' .. toInventory.label .. ' [' .. toInventory.id .. ']; ' .. fromData.count .. 'x ' .. fromData.name .. ' for ' .. toData.count .. 'x ' .. toData.name,
									sourceIdentifier = playerInventory.owner or toInventory.owner,
									event = 'Inventory:swapItems'
								}
								local additionalFields = {
									_type = 'Inventory:swapItems',
									_fromInventoryLabel = fromInventory.label, -- GetPlayerName(PlayerId())
									_fromInventoryID = (fromInventory.type == 'player' or fromInventory.type == 'drop') and fromInventory.id or '',
									_fromInventoryOwner = fromInventory.owner,
									_fromInventoryType = fromInventory.type,
									_toInventoryLabel = toInventory.label,
									_toInventoryID = (toInventory.type == 'player' or toInventory.type == 'drop') and toInventory.id or '',
									_toInventoryOwner = toInventory.owner,
									_toInventoryType = toInventory.type,
									_fromDataCount = fromData.count,
									_fromDataName = fromData.name,
									_toDataCount = toData.count,
									_toDataName = toData.name,
									_toDatametadata = toData.metadata,
									_fromData = fromData.metadata
								}
								TriggerEvent('NR_graylog:createLog', whData, additionalFields)
					...
					elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
						...
						if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
							...
							if not sameInventory then
								...
								local whData = {
									message = fromInventory.label .. ' [' .. fromInventory.id .. '] ' .. ' Transferred x' .. data.count .. ' ' .. toData.name .. ' to ' .. toInventory.label .. ' [' .. toInventory.id .. ']; ' .. fromData.count .. 'x ' .. fromData.name .. ' for ' .. toData.count .. 'x ' .. toData.name,
									sourceIdentifier = playerInventory.owner or toInventory.owner,
									event = 'Inventory:Stackitems'
								}
								local additionalFields = {
									_type = 'Inventory:Stackitems',
									_fromInventoryLabel = fromInventory.label, -- GetPlayerName(PlayerId())
									_fromInventoryID = (fromInventory.type == 'player' or fromInventory.type == 'drop') and fromInventory.id or '',
									_fromInventoryOwner = fromInventory.owner,
									_fromInventoryType = fromInventory.type,
					...
					elseif data.count <= fromData.count then
						...
						if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
							if not sameInventory then
								...
								local whData = {
									message = fromInventory.label .. ' [' .. fromInventory.id .. '] ' .. ' Transferred ' .. data.count .. 'x ' .. fromData.name .. ' to '.. toInventory.label .. ' [' .. toInventory.id .. ']; 剩餘: ' .. fromData.count - toData.count .. 'x ' .. fromData.name .. ' for ' .. toData.count .. 'x ' .. toData.name,
									sourceIdentifier = toInventory.owner or fromInventory.owner,
									event = 'Inventory:MoveItem2EmptySlot'
								}
								local additionalFields = {
									_type = 'Inventory:MoveItem2EmptySlot',
									_message = 'Move item to an empty slot',
									_fromInventoryLabel = fromInventory.label, -- GetPlayerName(PlayerId())
									_fromInventoryID = (fromInventory.type == 'player' or fromInventory.type == 'drop') and fromInventory.id or '',
									_fromInventoryOwner = fromInventory.owner,
									_fromInventoryType = fromInventory.type,
	...
	if toInventory.type == 'player' then
		...
		if not toInventory.open and Inventory.CanCarryItem(toInventory, item, count, data.metadata) then
			if data and data.count >= count then
				...
				local whData = {
					message = fromInventory.label .. ' [' .. fromInventory.id .. '] ' .. ' gave ' .. count .. 'x ' .. data.name .. ' to ' .. toInventory.label .. ' [' .. toInventory.id .. ']',
					sourceIdentifier = fromInventory.owner or toInventory.owner,
					event = 'ox_inventory:giveItem'
				}
				local additionalFields = {
					_type = 'Inventory:giveItem',
					_fromInventoryLabel = fromInventory.label, -- GetPlayerName(PlayerId())
					_fromInventoryID = fromInventory.id,
					_fromInventoryOwner = fromInventory.owner,
					_fromInventoryType = fromInventory.type,
					_toInventoryLabel = toInventory.label,
					_toInventoryID = toInventory.id,
					_toInventoryOwner = toInventory.owner,
					_toInventoryType = toInventory.type,
					_dataName = data.name,
					_dataCount = data.count,
...
lib.addCommand('group.admin', {'additem', 'giveitem'}, function(source, args)
	...
	if args.item and args.count > 0 then
		...
		local whData = {
			message = source.label .. ' [ ' .. source.id .. ' ] gave ' .. args.count .. 'x ' .. args.item.name .. ' to ' .. inventory.label .. ' [' .. inventory.id .. ']',
			sourceIdentifier = source.owner,
			event = 'CMD:giveItem'
		}
		local additionalFields = {
			_type = 'Inventory:CMD:giveItem',
			_sourceLabel = source.label, -- GetPlayerName(PlayerId())
			_sourceID = source.id,
			_sourceOwner = source.owner,
			_toInventoryLabel = inventory.label,
			_toInventoryID = inventory.id,
			_toInventoryOwner = inventory.owner,
			_itemName = args.item.name,
			_itemCount = args.count,
			_itemMetadata = args.metatype
		}
		TriggerEvent('NR_graylog:createLog', whData, additionalFields)
...
lib.addCommand('group.admin', 'removeitem', function(source, args)
	...
	if args.item and args.count > 0 then
		...
		source = Inventories[source] or {label = 'console', owner = 'console'}
		local whData = {
			message = source.label .. ' [' .. source.id .. '] ' .. ' took ' .. args.count .. 'x ' .. args.item.name .. ' from ' .. inventory.label .. ' [' .. inventory.id .. ']',
			sourceIdentifier = source.owner,
			event = 'CMD:removeitem'
		}
		local additionalFields = {
			_type = 'Inventory:CMD:removeitem',
			_sourceLabel = source.label, -- GetPlayerName(PlayerId())
			_sourceID = source.id,
			_sourceOwner = source.owner,
			_fromInventoryLabel = inventory.label,
			_fromInventoryID = inventory.id,
			_fromInventoryOwner = inventory.owner,
			_itemName = args.item.name,
			_itemCount = args.count,
			_itemMetadata = args.metatype
		}
...
lib.addCommand('group.admin', 'setitem', function(source, args)
	...
	if args.item and args.count >= 0 then
		...
		source = Inventories[source] or {label = 'console', owner = 'console'}
		local whData = {
			message = source.owner .. ' ' .. source.label .. ' set ' .. inventory.label .. ' [' .. inventory.id .. '] ' .. args.item.name .. ' count to ' .. args.count .. 'x',
			sourceIdentifier = source.owner,
			event = 'CMD:setitem'
		}
		local additionalFields = {
			_type = 'Inventory:CMD:setitem',
			_sourceLabel = source.label, -- GetPlayerName(PlayerId())
			_sourceID = source.id,
			_sourceOwner = source.owner,
			_fromInventoryLabel = inventory.label,
...
local ViewInv = function(source, targetid)
	local src = source
    local inventory = Inventories[targetid] or Inventories[tonumber(targetid)]
    TriggerClientEvent('ox_inventory:viewInventory', src, inventory)
end
exports('ViewInv', ViewInv)

lib.addCommand('group.admin', 'viewinv', function(source, args)
	ViewInv(source, args.id)
end, {'target'})
