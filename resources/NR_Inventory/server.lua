	...
	if data then
		...
		elseif type(data) == 'table' then
			if data.class and data.model then
				...
				if not right then
					local vehicleData = Vehicles[inv]['plates'][data.plate] or Vehicles[inv]['models'][data.model] or Vehicles[inv][data.class]
		...
		elseif inv == 'policeevidence' then
			if server.hasGroup(left, shared.police) then
				...
				if not right then
					right = Inventory.Create(data, shared.locale('police_evidence'), inv, 100, 0, 1000000, false)
				end
			end
