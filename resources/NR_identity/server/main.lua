...
if Config.UseDeferrals then
    AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
        ...
        if identifier then
            MySQL.query('SELECT dateofbirth, sex, height FROM users WHERE identifier = @identifier', {
                    ...
                    if result[1].dateofbirth then
                        ...
                        deferrals.presentCard(
                            ...
                            function(data, rawData)
                                if data.dateofbirth == '' or data.sex == '' or data.height == '' then
                                    deferrals.done(_U('data_incorrect'))
                                else
                                    if checkDOBFormat(data.dateofbirth) and checkSexFormat(data.sex) and
    ...
    ESX.RegisterServerCallback('esx_identity:registerIdentity', function(source, cb, data)
        ...
        if xPlayer then
            if not alreadyRegistered[xPlayer.identifier] then
                ...
                if checkSexFormat(data.sex) and checkDOBFormat(data.dateofbirth) and checkHeightFormat(data.height) then
                ...
                print(alreadyRegistered[xPlayer.identifier], not alreadyRegistered[xPlayer.identifier],
                    xPlayer.identifier)
                print("false 2")
                cb('alreadyRegistered')
            end
        else
            if multichar and checkSexFormat(data.sex) and checkDOBFormat(data.dateofbirth) and
                checkHeightFormat(data.height) then
...
function saveIdentityToDatabase(identifier, identity)
    MySQL.update.await(
        'UPDATE users SET dateofbirth = @dateofbirth, sex = @sex, height = @height WHERE identifier = @identifier', {
            ['@identifier'] = identifier,
...
function deleteIdentityFromDatabase(xPlayer)
    MySQL.update.await(
        'UPDATE users SET dateofbirth = @dateofbirth, sex = @sex, height = @height , skin = @skin WHERE identifier = @identifier',
        {
            ['@identifier'] = xPlayer.identifier,
            ['@dateofbirth'] = NULL,
            ['@sex'] = NULL,
    ...
    if Config.FullCharDelete then
        MySQL.update.await(
            'UPDATE addon_account_data SET money = 0 WHERE account_name = @account_name AND owner = @owner', {
