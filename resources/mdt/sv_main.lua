...
RegisterCommand("mdt", function(source)
    ...
    if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
        TriggerClientEvent('np-mdt:open', src, rank, xPlayer.name)
    end
end)
...
function getVehicles(src)
    local result = MysqlConverter(Config.Mysql, 'fetchAll', 'SELECT * FROM bbvehicles aa LEFT JOIN vehicle_mdt a ON a.license_plate = aa.plate LEFT JOIN ____mdw_bolos at ON at.license_plate = aa.plate ORDER BY time ASC', {})
    for k, v in pairs(result) do
        ...
        local owner = MysqlConverter(Config.Mysql, 'fetchAll', 'SELECT citizenid FROM bbvehicles WHERE plate = @plate', {['@plate'] = v.plate })
        local owner_name = MySQL.prepare.await("SELECT name FROM users WHERE identifier = ?", {owner[1].citizenid})
        result[k].owner = owner[1].citizenid
        result[k].cid = owner_name
...
AddEventHandler("np-mdt:getProfileData", function(identifier)
    ...
    TriggerClientEvent("np-mdt:getProfileData", src, data)
end)

RegisterServerEvent("np-mdt:getVehicleData")
AddEventHandler("np-mdt:getVehicleData", function(plate)
    local src = source
	local result = MysqlConverter(Config.Mysql, 'fetchAll', 'SELECT * FROM bbvehicles aa LEFT JOIN vehicle_mdt a ON a.license_plate = aa.plate LEFT JOIN ____mdw_bolos at ON at.license_plate = aa.plate WHERE aa.plate = @plate LIMIT 1', {['@plate'] = plate })
    for k, v in pairs(result) do
        if v.image and v.image ~= nil and v.image ~= "" then
            result[k].image = v.image
        else
...
AddEventHandler("np-mdt:searchVehicles", function(plate)
	...
	local result = MysqlConverter(Config.Mysql, 'fetchAll', 'SELECT * FROM bbvehicles aa LEFT JOIN vehicle_mdt a ON a.license_plate = aa.plate LEFT JOIN ____mdw_bolos at ON at.license_plate = aa.plate WHERE LOWER(plate) LIKE @plate ORDER BY plate ASC', { ['@plate'] = lowerplate })
    for k, v in pairs(result) do
        if v.image and v.image ~= nil and v.image ~= "" then
            result[k].image = v.image
        else
...
AddEventHandler("np-mdt:searchProfile", function(query)
    ...
    local result = MysqlConverter(Config.Mysql, 'fetchAll', "SELECT * FROM `users` WHERE LOWER(`name`) LIKE @var1 OR LOWER(`identifier`) LIKE @var3 OR CONCAT(LOWER(`name`), ' ', LOWER(`identifier`)) ORDER BY name DESC", {
        ['@var1'] = queryData,
        ['@var2'] = queryData,
        ['@var3'] = queryData
    })
	...
	for k, v in pairs(result) do
        ...
        local drive_truck = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'drive_truck'})
        if drive_truck and drive_truck[1] then
            if drive_truck[1].status == 1 then
                result[k].DriverTruck = true
            end
        end

        local drivers2 = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'drive'})
        ...
        local dmv = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'dmv'})
        if dmv and dmv[1] then
            if dmv[1].status == 1 then
                result[k].DMV = true
            end
        end

        local boating = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'boating'})
        if boating and boating[1] then
            if boating[1].status == 1 then
                result[k].Boating = true
            end
        end

        local driver_motor = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'driver_motorcycle'})
        if driver_motor and driver_motor[1] then
            if driver_motor[1].status == 1 then
                result[k].Motorbike = true
            end
        end

        local pilot2 = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'aircraft'})
...
function getProfile(identifier)
    ...
    local vehresult = MySQL.query.await("SELECT * FROM bbvehicles WHERE citizenid = ?", {identifier})
...
function getProfiles(src)
    local result = MySQL.query.await("SELECT aa.name, aa.firstname, aa.lastname, aa.identifier, at.image FROM users aa LEFT JOIN ___mdw_profiles at ON at.cid = aa.identifier")
    -- local result =  MysqlConverter(Config.Mysql, 'fetchAll', 'SELECT * FROM users aa LEFT JOIN ___mdw_profiles at ON at.cid = aa.identifier', {})
	for k, v in pairs(result) do
        result[k].name = v.name
        result[k].firstname = v.firstname
        result[k].lastname  = v.lastname
		local weapon = MysqlConverter(Config.Mysql, 'fetchAll','SELECT status FROM user_licenses WHERE owner = @owner AND type = @lictype', {['@owner'] = identifier, ['@lictype'] = 'Weapon'})
...
AddEventHandler("np-mdt:updateLicense", function(identifier, type, status)
    ...
    TriggerEvent("np-mdt:newLog", name .. " " .. action .. " licenses type: " .. firstToUpper(type) .. " Edited Citizen Id: " .. identifier, time)
...
AddEventHandler("np-mdt:incidentSearchPerson", function(query1)
	...
	local result = MysqlConverter(Config.Mysql, 'fetchAll', "SELECT name, identifier FROM `users` WHERE LOWER(`name`) LIKE @var1 OR LOWER(`identifier`) LIKE @var2 OR CONCAT(LOWER(`name`), ' ', LOWER(`identifier`)) LIKE @var3", {
		["@var1"] = queryData,
		["@var2"] = queryData,
		["@var3"] = queryData
	})
...
AddEventHandler("np-mdt:removeIncidentCriminal", function(cid, icId)
    ...
    TriggerEvent("np-mdt:newLog", name .. ", " .. action .. ", Criminal Citizen Id: " .. cid .. ", Name: " .. Cname .. "", time)
...
AddEventHandler("np-mdt:saveIncident", function(data)
    ...
    for i = 1, #data.associated do
        ...
        data.associated[i].name = result2[1].name
       end
    end
    if data.ID ~= 0 then
...
AddEventHandler("np-mdt:newReport", function(data)
    ...
    local result = MysqlConverter(Config.Mysql, 'fetchAll' ,'SELECT * FROM `____mdw_reports` WHERE `dbid` = @id', {
        ...
        if data.id ~= nil and data.id ~= 0 then
            ...
            TriggerEvent("np-mdt:newLog", name .. ", " .. action .. ", Changes: " .. json.encode(data), data.time)
