...
AddEventHandler('np-mdt:open', function(jobLabel, name)
    open = true
    EnableGUI(open)
    ...
    SendNUIMessage({ type = "data", name = "歡迎, " ..jobLabel..' '..name, location = playerStreetsLocation, fullname = name })
end)
...
RegisterNUICallback("newBolo", function(data, cb)
    print(data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
...
AddEventHandler('np-mdt:searchVehicles', function(sentData)
    for i=1, #sentData do
        local vehicle = json.decode(sentData[i]['props'])
...
AddEventHandler('np-mdt:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        local vehicle = sentData[1]
        local vehData = json.decode(vehicle['props'])
        vehicle["dbid"] = vehicle["id"]
