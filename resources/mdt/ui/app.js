...
function timeAgo(dateParam) {
    ...
    if (seconds < 5) {
        ...
        return getFormattedDate(date, '今日');
    } else if (isYesterday) {
        return getFormattedDate(date, '昨日');
    } else if (isThisYear) {
...
$(document).ready(() => {
    ...
    $(".nav-item").click(function() {
        console.log($(this).hasClass("active-nav") + " THIS TRUE?")
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-save", function() {
        console.log(canSaveProfile + "  :canSaveProfile")
        if (canSaveProfile == true) {
            canSaveProfile = false
            ...
            setTimeout(() => {
                if (canRefreshIncidents == true) {
                    ...
                    setTimeout(() => {
                        $(".incidents-search-refresh").empty();
                        $(".incidents-search-refresh").html("刷新");
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function() {
        let template = "概括:\n\n[ 輸入報告摘要 ]\n\n人質: [ 名稱 ]\n\n沒收的武器/物品:\n\n· [ 列出所有沒收的武器/物品 ]\n\n-----\n發票:\n監禁:\n-----"
        $("#manage-incidents-title-input").val("名稱 - 控罪 - " + $(".date").html());
        $(".manage-incidents-reports-content").val(template);
        ...
        $(".manage-incidents-title-holder").prepend(
            `
            <div class="manage-incidents-title">管理案件</div>
        ...
        $(".manage-incidents-editing-title").html("你正在創建新的案件");
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-new", function() {
        //if ($(".manage-bolos-editing-title").html() == '你正在創建新的BOLO') {
        ...
        $(".manage-bolos-editing-title").html("你正在創建新的BOLO")
        ...
        $(".manage-bolos-editing-title").data("id", 0);
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-save", function() {
        let existing = !($(".manage-bolos-editing-title").html() == '你正在創建新的BOLO')
        ...
        $(".incidents-ghost-holder").prepend(
            ...
            <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}">
                ...
                <div class="associated-incidents-user-tags-holder">
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">通緝令</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">罪成</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">已處理</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">關聯</div>
                </div>
                ...
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">建議監禁時間</div>
                ...
                <div class="associated-incidents-fine-input" data-id="${$(this).data("cid")}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="輸入罰款..." value="0" class="fine-amount" data-id="${$(this).data("cid")}" type="number"></div>
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">監禁時間</div>
                <div class="associated-incidents-sentence-input" data-id="${$(this).data("cid")}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="輸入監禁時間..." value="0" class="sentence-amount" data-id="${$(this).data("cid")}" type="number"></div>
            </div>
            `
        );
    });
    ...
    $(".reports-search-refresh").click(function() {
        if (canRefreshReports == true) {
            ...
            setTimeout(() => {
                $(".reports-search-refresh").empty();
                $(".reports-search-refresh").html("刷新");
    ...
    $(".dispatch-comms-refresh").click(function() {
        ...
        setTimeout(() => {
            $(".dispatch-comms-refresh").empty();
            $(".dispatch-comms-refresh").html("刷新");
            canRefreshReports = true
    ...
    $(".manage-reports-title-holder").on("click", ".manage-reports-new", function() {
        ...
        //} else {
        if ($(".badge-logo").attr('src') == 'img/ems_badge.png') {
            template = "提交至ICU?: [Yes/No]\n\案件報告:\n[ 簡要總結發生了什麼以及誰在現場做了什麼。注意現場的任何突出之處以及為治療患者所做的工作 ]\n\n受傷名單:\n- [ 說明受了什麼傷害 ]\n\n💉 手術報告:\n[ 關於手術過程的完整報告，列出任何病發症或手術中發現的任何事情。注意誰參與手術以及他們在手術過程中做了什麼。在報告的最後一定要記下病人的狀態 ]\n\n參與人員:\n- [ 在這裡列出所有參與人員 ]\n\n藥物:\n- [ 在這裡列出所有藥物 ]\n\n備註:\n[ 備註信息 ]"
        }
        $(".manage-reports-editing-title").html("你現正創建新的報告")
        $(".manage-reports-input-title").val('');
        $(".manage-reports-input-type").val('');
        $(".manage-reports-reports-content").val(template);
        $(".manage-reports-editing-title").data("id", 0);
    ...
    $(".manage-reports-title-holder").on("click", ".manage-reports-save", function() {
        let existing = !($(".manage-reports-editing-title").html() == '你現正創建新的報告')
    ...
    function JobColors(sentJob) {
        if (sentJob) {
            if (sentJob == 'police') {
                ...
                $(".header-title").html("Nathanroad POL");
                $(".bolo-nav-item").html("BOLOs");
                $(".bolos-search-title").html("Bolos");
                $("#bolos-search-input").attr('placeholder', '搜尋 Bolo...');
                $(".manage-bolos-title").html("管理Bolo");
                $(".manage-bolos-editing-title").html("你正在創建新的BOLO");
                $(".boloplate-title").html("車牌");
                $(".boloowner-title").html("擁有者");
                $(".boloindividual-title").html("個體");
                $("#boloplate").attr('placeholder', '輸入車牌...');
                $("#bolodetail").attr('placeholder', 'Bolo詳細資料...');
                $("#boloowner").attr('placeholder', '輸入擁有者...');
                $("#boloindividual").attr('placeholder', '輸入個體...');
                ...
                $(".bolo-nav-item").show()
                $(".dmv-nav-item").show()
                $(".dispatch-title-ofsomesort").html("任務對話")
                ...
                $("#reports-officers-involved-tag-title").html("負責警員")
                ...
                $(".header-title").html("Nathanroad Medical Department");
                $(".quote-span").html("The simplest explanation is almost always somebody screwed up.");
                $(".bolo-nav-item").html("ICU");
                $(".bolos-search-title").html("ICU 登記");
                $("#bolos-search-input").attr('placeholder', '搜尋登記...');
                $(".manage-bolos-title").html("管理 ICU 登記");
                $(".manage-bolos-editing-title").html("你正在創建新的ICU登記");
                $(".boloplate-title").html("預計康復");
                $(".boloowner-title").html("緊急聯絡人");
                $(".boloindividual-title").html("病人");
                $("#boloplate").attr('placeholder', '輸入康復時間...');
                $("#bolodetail").attr('placeholder', '輸入ICU登記詳細資料...');
                $("#boloowner").attr('placeholder', '輸入緊急聯絡人...');
                $("#boloindividual").attr('placeholder', '輸入病人名稱和CID...');
                $(".incidents-nav-item").hide()
                $(".dmv-nav-item").hide()
                $("#reports-officers-involved-tag-title").html("負責醫護人員")
                $(".dispatch-title-ofsomesort").html("任務對話")
                $(".dispatch-comms-container").fadeIn(0)
                $(".manage-profile-name-input-1").attr("readonly", true)
                $(".manage-profile-name-input-2").attr("readonly", true)
    ...
    window.addEventListener('message', function(event) {
        ...
        if (e.type == "show") {
            ...
            $(".manage-profile-editing-title").html("你正在編輯 " + table["name"])
            $(".manage-profile-citizenid-input").val(table["identifier"]);
            $(".manage-profile-name-input-1").val(table["name"])
            $(".manage-profile-name-input-2").val("N/A")
            ...
            let boating = ""
            if (table["Boating"] == true) {
                boating = "green-tag"
            } else {
                boating = "red-tag"
            }
            let drive_truck = ""
            if (table["DriverTruck"] == true) {
                drive_truck = "green-tag"
            } else {
                drive_truck = "red-tag"
            }
            let driver_motor = ""
            if (table["Motorbike"] == true) {
                driver_motor = "green-tag"
            } else {
                driver_motor = "red-tag"
            }
            $(".licenses-holder").empty();
            $('.licenses-holder').prepend(
                `

            <div class="license-tag ${driver} driver">駕駛</div>
            <div class="license-tag ${weapon} weapon">槍械</div>
            <div class="license-tag ${dmv} dmv">筆試</div>
            <div class="license-tag ${boating} boating">船</div>
            <div class="license-tag ${pilot} pilot">飛行</div>
            <div class="license-tag ${drive_truck} drive_truck">貨車</div>
            <div class="license-tag ${driver_motor} driver_motor">電單車</div>
            `);
            $(".tags-holder").empty();
            ...
            $.each(table, function(index, value) {
                ...
                if (value.Boating == true) {
                    boating = "green-tag";
                } else {
                    boating = "red-tag";
                }
                if (value.DriverTruck == true) {
                    drive_truck = "green-tag";
                } else {
                    drive_truck = "red-tag";
                }
                if (value.Motorbike == true) {
                    driver_motor = "green-tag";
                } else {
                    driver_motor = "red-tag";
                }
                ...
                <div class="profile-tag ${driver}">駕駛</div>
                <div class="profile-tag ${weapon}">槍械</div>
                <div class="profile-tag ${dmv}">筆試</div>
                <div class="profile-tag ${boating}">船</div>
                <div class="profile-tag ${pilot}">飛行</div>
                <div class="profile-tag ${drive_truck}">貨車</div>
                <div class="profile-tag ${driver_motor}">電單車</div>
                </div>
                </div>
            ...
            console.log("dashboardCalls calls")
            const table = e.data
            ...
            $.each(table, function(index, value) {
                $('.incidents-items').prepend(
                    ...
                    <div class="incidents-top-holder">
                        ...
                        <div class="incedent-report-name">案件報告</div>
                    </div>
            ...
            $.each(penalcode, function(index, value) {
                // console.log(value)
                $(`#penal-${index}`).append(`
                    <div class="offense-item ${value.color}-penis-code" data-sentence="${value.months}" data-fine="${value.fine}">
                    <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-top: 0vh;">
                        <div class="offense-item-offense">${value.title}</div>
                        <div class="offfense-item-name">${value.class}</div>
                    </div>
                    <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-bottom: 0vh; padding-top: 0.75vh;">
                        <div class="offense-item-id">${value.id}</div>
                        <div class="offfense-item-months">${value.months} Months - $${value.fine} - 0 Points(s)</div>
                    </div>
                    `)
                    console.log(index)
            })
            ...
            $(".incidents-ghost-holder").html("")
            $(".associated-incidents-tags-holder").html("")
            $(".manage-incidents-editing-title").html("你現正創建新的案件 " + table["id"]);
                ...
                if (value.associated == true) {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="associated-incidents-user-tags-holder">
                                <div class="associated-incidents-user-tag ${warrantTag}" data-id="${value.cid}">通緝令</div>
                                <div class="associated-incidents-user-tag ${guiltyTag}" data-id="${value.cid}">罪成</div>
                                <div class="associated-incidents-user-tag ${processedTag}" data-id="${value.cid}">已處理</div>
                                <div class="associated-incidents-user-tag ${associatedTag}" data-id="${value.cid}">關聯</div>
                            </div>
                            <div class="associated-incidents-user-holder" data-name="${value.cid}" style="display:none;">
                            </div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">建議罰款</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">建議監禁時間</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">罰款</div>
                            <div class="associated-incidents-fine-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="輸入罰款..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">監禁時間</div>
                            <div class="associated-incidents-sentence-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="輸入監禁時間..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                        </div>`
                    );
                } else {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">建議罰款</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">建議監禁時間</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">罰款</div>
                            <div class="associated-incidents-fine-input" data-id="${value.cid}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="輸入罰款..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">監禁時間</div>
                            <div class="associated-incidents-sentence-input" data-id="${value.cid}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="輸入監禁時間..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                        </div>`
                    );
                }
                ...
                $(".icidents-person-search-holder").prepend(
                    ...
                    <div class="icidents-person-search-item" data-info="${name} (#${value.identifier})" data-cid="${value.identifier}" data-name="${name}">
                        ...
                        <div class="icidents-person-search-item-right">
                            <div class="icidents-person-search-item-right-cid-title">市民 ID</div>
                            <div class="icidents-person-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.identifier}</div>
                            <div class="icidents-person-search-item-right-name-title">名稱</div>
            ...
            let table = e.data
            $(".manage-bolos-editing-title").html("你正在編輯BOLO " + table["dbid"]);

            if ($(".badge-logo").attr('src') == 'img/ems_badge.png') {
                $(".manage-bolos-editing-title").html("你正在編輯ICU登記 " + table["dbid"]);
            }
            ...
            $(".manage-reports-editing-title").html("你正在編輯報告 " + id);
            ...
            $(".manage-reports-editing-title").html("你正在編輯報告 " + table["dbid"]);
            ...
            $(".vehicle-info-owner-input").val(table["cid"]);
            ...
            $('.vehicle-tags').append(`<div class="vehicle-tag ${impound}">扣押</div>`);
            ...
            $('.vehicle-tags').append(`<div class="vehicle-tag ${stolen} stolen-tag">偷取</div>`);
            $(".vehicle-info-imageurl-input").val(table["image"]);
            ...
            $.each(lspd, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    ...
                    <div class="unit-job active-info-job-lspd">NRP</div>
            ...
            $.each(ems, function(index, value) {
                ...
                <div class="active-unit-item" data-id="${value.cid}">
                    <div class="unit-status ${statuscolor}">${status}</div>
                    <div class="unit-job active-info-job-ambulance">Medic</div>
                    <div class="unit-name">(${callsign}) ${value.name}</div>
                    <div class="unit-radio">${radio}</div>
                </div>
                `)
