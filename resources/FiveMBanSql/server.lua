...
CreateThread(function()
    while Config.MultiServerSync do
        Wait(30000)
        MySQL.query('SELECT * FROM banlist', {}, function(data)
...
RegisterCommand('sqlbanreload', function(source)
...
RegisterCommand('sqlbanhistory', function(source, args, user)
    if args[1] and BanListHistory and havePermission(source) then
        ...
        if name ~= "" then
            ...
            if nombre and nombre > 0 then
                ...
                local resultat = tostring(BanListHistory[nombre].targetplayername .. " , " ..
                                              BanListHistory[nombre].sourceplayername .. " , " ..
                                              BanListHistory[nombre].reason .. " , " .. calcul2 .. Text.day .. " , " ..
                                              BanListHistory[nombre].added)
                ...
                for i = 1, #BanListHistory, 1 do
                    if (tostring(BanListHistory[i].targetplayername)) == tostring(name) then
                        ...
                        local calcul2 = calcul1 / 86400
                        local calcul2 = math.ceil(calcul2)
                        local resultat = tostring(BanListHistory[i].targetplayername .. " , " ..
                                                      BanListHistory[i].sourceplayername .. " , " ..
                                                      BanListHistory[i].reason .. " , " .. calcul2 .. Text.day .. " , " ..
                                                      BanListHistory[nombre].added)
...
RegisterCommand('sqlunban', function(source, args, user)
    if args[1] and havePermission(source) then
        local target = table.concat(args, " ")
        MySQL.query('SELECT * FROM banlist WHERE targetplayername like @playername', {
            ['@playername'] = ("%" .. target .. "%")
        }, function(data)
            if data[1] then
                if #data > 1 then
                    ...
                    MySQL.query('DELETE FROM banlist WHERE targetplayername = @name', {
                        ['@name'] = data[1].targetplayername
                    }, function()
                        loadBanList()
                        if Config.EnableDiscordLink then
                            local sourceplayername = GetPlayerName(source)
                            local message = (data[1].targetplayername .. Text.isunban .. " " .. Text.by .. " " ..
                                                sourceplayername)
            ...
            if target and target > 0 then
                ...
                if ping and ping > 0 then
                    if duree and duree < 365 then
                        ...
                        if duree > 0 then
                            ban(source, identifier, license, liveid, xblid, discord, playerip, targetplayername,
                                sourceplayername, duree, reason, permanent)
...
RegisterCommand('sqlreason', function(source, args, user)
    ...
    if name ~= "" then
        if duree and duree < 365 then
            ...
            MySQL.query('SELECT * FROM baninfo WHERE playername = @playername', {
                ['@playername'] = name
            }, function(data)

                if data[1] then
                    if duree > 0 then
                        ban(source, data[1].identifier, data[1].license, data[1].liveid, data[1].xblid, data[1].discord,
                            data[1].playerip, name, sourceplayername, duree, reason, permanent)
                        lastduree = ""
                        lasttarget = ""
                    else
                        local permanent = 1
...
RegisterCommand('sqlbanoffline', function(source, args, user)
    if args ~= "" then
        ...
        if lastduree ~= "" then
            if target ~= "" then
                MySQL.query('SELECT * FROM baninfo WHERE playername like @playername', {
                    ['@playername'] = ("%" .. target .. "%")
                }, function(data)
                    if data[1] then
                        if #data > 1 then
                            ...
                            TriggerEvent('bansql:sendMessage', source,
                                (lasttarget .. Text.during .. lastduree .. Text.forcontinu))
                        end
                    else
                        TriggerEvent('bansql:sendMessage', source, Text.invalidname)
                    end
...
RegisterCommand('sqlbanid', function(source, args, user)
    if args ~= "" then
        lastduree = tonumber(args[1])
        -- target     = args[2]
        if lastduree ~= "" then
            if target ~= "" then
                MySQL.query('SELECT * FROM baninfo WHERE identifier = @identifier', {
                    ['@identifier'] = args[2]
                }, function(data)
                    if data[1] then
                        if #data > 1 then
                            TriggerEvent('bansql:sendMessage', source, Text.toomanyresult)
                            for i = 1, #data, 1 do
                                TriggerEvent('bansql:sendMessage', source, data[i].identifier)
                            end
                        else
                            lasttargetid = data[1].identifier
                            TriggerEvent('bansql:sendMessage', source,
                                (lasttargetid .. Text.during .. lastduree .. Text.forcontinuid))
                        end
                    else
                        TriggerEvent('bansql:sendMessage', source, Text.invalidname)
                    end
...
RegisterCommand('sqlreasonid', function(source, args, user)
    if source ~= 0 then
        local duree = lastduree
        local identifier = lasttargetid
        local reason = table.concat(args, " ", 1)
        local permanent = 0
        local playerip = "0.0.0.0"
        local liveid = "no info"
        ...
        if identifier ~= "" then
            if duree and duree < 365 then
                ...
                MySQL.query('SELECT * FROM baninfo WHERE identifier = @identifier', {
                    ...
                    if data[1] then
                        if duree > 0 then
                            ban(source, identifier, data[1].license, data[1].liveid, data[1].xblid, data[1].discord,
                                data[1].playerip, data[1].playername, sourceplayername, duree, reason, permanent)
                            lastduree = ""
                            lasttarget = ""
                        else
                            local permanent = 1
...
function ban(source, identifier, license, liveid, xblid, discord, playerip, targetplayername, sourceplayername, duree,
    reason, permanent)
    ...
    if permanent == 0 then
        ...
        message = (identifier .. " " .. license .. " " .. liveid .. " " .. xblid .. " " .. discord .. " " .. playerip ..
                      " " .. targetplayername .. Text.isban .. " " .. duree .. Text.forr .. reason .. " " .. Text.by ..
                      " " .. sourceplayername)
    else
        TriggerEvent('bansql:sendMessage', source, (Text.youban .. targetplayername .. Text.permban .. reason))
        message = (identifier .. " " .. license .. " " .. liveid .. " " .. xblid .. " " .. discord .. " " .. playerip ..
                      " " .. targetplayername .. Text.isban .. " " .. Text.permban .. reason .. " " .. Text.by .. " " ..
                      sourceplayername)
    end
    if Config.EnableDiscordLink then
...
function loadBanListHistory()
    MySQL.query('SELECT * FROM banlisthistory', {}, function(data)
        BanListHistory = {}
...
function deletebanned(identifier)
    MySQL.query('DELETE FROM banlist WHERE identifier=@identifier', {
        ['@identifier'] = identifier
    }, function()
        loadBanList()
    end)
...
AddEventHandler('playerConnecting', function(playerName, setKickReason)
    ...
    for i = 1, #BanList, 1 do
        if ((tostring(BanList[i].identifier)) == tostring(steamID) or (tostring(BanList[i].license)) ==
            tostring(license) or (tostring(BanList[i].liveid)) == tostring(liveid) or (tostring(BanList[i].xblid)) ==
            ...
            elseif (tonumber(BanList[i].expiration)) > os.time() then
                ...
                if tempsrestant >= 1440 then
                    ...
                    setKickReason(Text.yourban .. BanList[i].reason .. Text.timeleft .. txtday .. Text.day .. txthrs ..
                                      Text.hour .. txtminutes .. Text.minute)
                    CancelEvent()
                    break
...
AddEventHandler('esx:playerLoaded', function(source)
    CreateThread(function()
        ...
        MySQL.query('SELECT * FROM `baninfo` WHERE `identifier` = @identifier', {
            ...
            if not found then
                MySQL.update(
                    'INSERT INTO baninfo (identifier,license,liveid,xblid,discord,playerip,playername) VALUES (@identifier,@license,@liveid,@xblid,@discord,@playerip,@playername)',
                    {
                        ['@identifier'] = steamID,
                        ['@license'] = license,
                        ['@liveid'] = liveid,
                ...
                MySQL.update(
                    'UPDATE `baninfo` SET `license` = @license, `liveid` = @liveid, `xblid` = @xblid, `discord` = @discord, `playerip` = @playerip, `playername` = @playername WHERE `identifier` = @identifier',
                    {
                        ['@identifier'] = steamID,
                        ['@license'] = license,
                        ['@liveid'] = liveid,
