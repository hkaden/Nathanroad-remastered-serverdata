...
function setJobPriaveFrequencyAccess()
	for index, channel in pairs(radioConfig.Frequency.Private) do
		print("revmove " .. index)
		RemovePlayerAccessToFrequency(index)
	end
	if PlayerData.job and radioConfig.Frequency.JobFrequency[PlayerData.job.name] then
		GivePlayerAccessToFrequencies(radioConfig.Frequency.JobFrequency[PlayerData.job.name])
		print("added to " .. PlayerData.job.name)
	end
end
...
Radio.Labels = {
	{ "FRZL_RADIO_HELP", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " 隱藏~n~~" .. radioConfig.Controls.Toggle.Name .. "~ ~g~開啟~s~ 無線電~n~~" .. radioConfig.Controls.Decrease.Name .. "~ 或 ~" .. radioConfig.Controls.Increase.Name .. "~ 切換頻道~n~~" .. radioConfig.Controls.Input.Name .. "~ 選擇頻道~n~~" .. radioConfig.Controls.ToggleClicks.Name .. "~ ~a~發話~n~頻道: ~1~ MHz" },
	{ "FRZL_RADIO_HELP2", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " 隱藏~n~~" .. radioConfig.Controls.Toggle.Name .. "~ ~r~關閉~s~無線電~n~~" .. radioConfig.Controls.Broadcast.Name .. "~ 廣播~n~頻道: ~1~ MHz" },
	{ "FRZL_RADIO_INPUT", "輸入頻道" },
}
...
Radio.Commands = {
	{
		...
		Help = "切換無線電界面", -- Command help shown in chatbox when typing the command
		Params = {},
	...
	},
	{
		...
		Help = "更變頻道", -- Command help shown in chatbox when typing the command
		Params = {
			{name = "number", "輸入頻道"}
		},
		Handler = function(src, args, raw)
	...
	while true do
		...
		if Radio.Open then
			...
			if not Radio.On then
				AddTextComponentSubstringPlayerName(Radio.Clicks and "~r~停用~w~" or "~g~啟用~w~")
			end
