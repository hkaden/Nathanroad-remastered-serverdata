			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						...
						payment_status = `<button type="button" id="" class="btn btn-blue flex-grow-1 payInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-shopping-bag"></i> 付款 $${invoices.invoice_value.toLocaleString()};</button>`;
					}
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												...
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">稅款 (${event.data.VAT}%)</span> <span class="" id="" style="font-size: 18px;">$${Math.round(invoices.invoice_value * (event.data.VAT/100)).toLocaleString()};</span>
												</div>
												<br>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px; font-weight: 700;">總額</span> <span class="" id="" style="font-size: 18px;">$${Math.round(invoices.invoice_value).toLocaleString()};</span>
												</div>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 16px; font-weight: 700;">注意</span> <span class="" id="" style="font-size: 14px;">請於三日內繳付賬單，逾期後總額每天都會疊加10%;</span>
												</div>
												<br>
...
$(document).on('click', ".payInvoice", function() {
	...
	$.post('https://NR_Billing/action', JSON.stringify ({
		action: "payInvoice",
	...
	if(!invoice_value || !invoice_item) {
		...
		if(invoice_value >= 0) {
			...
			$.post('http://NR_Billing/action', JSON.stringify({
				...
				invoice_notes: "備註: " + invoice_notes
			}));
