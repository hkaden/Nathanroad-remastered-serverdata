window.APP = {
	...
	mounted() {
		post('http://NR_Chat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if(this.message !== '') {
				post('http://NR_Chat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://NR_Chat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
			this.showInput = false;
			clearInterval(this.focusTimer);
